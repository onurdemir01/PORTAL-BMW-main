...
const SEED_LINKS = [
  ...
  },
  {
    ...
    category: 'Vendors',
    description: 'Red Hat resmi kaynaklar.',
    isActive: true,
    order: 4,
    openInNewTab: true,
    visibleTo: ['Admin', 'User'],
  ...
  {
    id: '6',
    label: 'AWX/Ansible Tower',
    url: 'https://maestro2',
    category: 'Automation',
    description: 'Ansible AWX job ve template yönetimi.',
    isActive: true,
    order: 5,
    openInNewTab: true,
    visibleTo: ['Admin'],
  ...
  {
    id: '7',
    label: 'LogX Dashboard',
    url: '/logx',
    category: 'Portal',
    description: 'Sunucu log viewer.',
    isActive: true,
    order: 6,
    openInNewTab: false,
    visibleTo: ['Admin', 'User'],
...
function normalizeLink(link) {
  return {
    ...link,
    visibleTo:
      Array.isArray(link.visibleTo) && link.visibleTo.length > 0
        ? link.visibleTo
        : ['Admin', 'User'],
...
function rowToLink(r) {
  return normalizeLink({
    ...
    icon: r.icon || '',
    visibleTo: String(r.visible_to || 'Admin,User')
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
...
async function insertLinkRow(l) {
  await db.query(
    `INSERT INTO portal_links (id, label, url, category, description, is_active, sort_order, open_in_new_tab, icon, visible_to, is_favorite, purpose, how_to_use)
    ...
    [
      ...
      l.order ?? 0,
      l.openInNewTab !== false ? 1 : 0,
      l.icon || '',
      (l.visibleTo || ['Admin', 'User']).join(','),
      l.isFavorite ? 1 : 0,
      l.purpose || '',
      l.howToUse || '',
    ],
...
async function importLegacyIfEmpty() {
  ...
  try {
    if (fs.existsSync(LEGACY_DUTY_LINKS_FILE)) {
      ...
      try {
        fs.renameSync(LEGACY_DUTY_LINKS_FILE, `${LEGACY_DUTY_LINKS_FILE}.migrated`);
      } catch {
        /* yoksay */
      }
    }
  ...
  for (const l of links) {
    try {
      await insertLinkRow(l);
    } catch (e) {
      console.warn(`[Links] import satiri eklenemedi (${l.label}):`, e.message);
    }
  }
...
// ── URL KAPISI ───────────────────────────────────────────────────────────────
//
// GUVENLIK ACIGI (bu PR'da kapatildi): URL yalnizca "bos mu" diye kontrol
// ediliyordu. Onyuz degeri DOGRUDAN `href`e basiyor (ImportantLinksPage), yani
// `javascript:...` kaydedilebiliyor ve TIKLAYAN HERKESTE portal origin'inde
...
//
// IZIN VERILENLER:
//   * https: / http:      — normal dis baglantilar
//   * /ile baslayan yol   — portal ici (seed'de `/logx` satiri VAR, kirilmamali)
// REDDEDILENLER: javascript:, data:, vbscript:, file: ve protokol-goreli `//host`
// (sonuncusu sema mirasi aldigi icin `https://` gibi gorunur ama baska bir
// origin'e gider).
const ALLOWED_URL_SCHEMES = new Set(['http:', 'https:']);

function urlError(raw) {
  const url = String(raw || '').trim();
  if (!url) return 'url gerekli';

  // Portal ici goreli yol. `//host` PROTOKOL-GORELIDIR ve dis bir origin'e gider —
  // tek `/` ile baslayan gercek goreli yoldan ayrilmali.
  if (url.startsWith('/')) {
    if (url.startsWith('//')) return 'url: protokol-göreli adres (//) kabul edilmiyor';
    return null;
  }
  ...
  try {
    ...
    return 'url: geçerli bir adres değil (https://... ya da /portal-yolu bekleniyor)';
  }
  if (!ALLOWED_URL_SCHEMES.has(parsed.protocol)) {
    return `url: "${parsed.protocol}" şeması kabul edilmiyor (yalnızca http, https ya da /portal-yolu)`;
  }
  return null;
}
...
function initLinks(app) {
  ...
  try {
    router.use(require('../audit/index.cjs').auditMutations('links'));
  } catch {
    /* yoksay */
  }
  ...
  router.post('/', async (req, res) => {
    ...
    const newLink = normalizeLink({
      ...
      // NE ISE YARAR / NASIL KULLANILIR: kartta kisa aciklama, ayrintida bu ikisi.
      // Kullanici istegi: "islev ne ise yarar nasil kullanilir vs yazsin".
    ...
    try {
      await db.query(
        `UPDATE portal_links SET label=$1, url=$2, category=$3, description=$4, is_active=$5,
           open_in_new_tab=$6, icon=$7, visible_to=$8, is_favorite=$9, purpose=$10,
           how_to_use=$11, updated_at=GETUTCDATE()
         WHERE id=$12`,
        [
          updated.label,
          updated.url,
          ...
          updated.openInNewTab ? 1 : 0,
          updated.icon || '',
          updated.visibleTo.join(','),
  ...
  router.post('/reorder', async (req, res) => {
    ...
    if (!Array.isArray(orderedIds))
      return res.status(400).json({ ok: false, error: 'orderedIds array gerekli.' });
    try {
      for (let i = 0; i < orderedIds.length; i++) {
        await db.query(
          `UPDATE portal_links SET sort_order=$1, updated_at=GETUTCDATE() WHERE id=$2`,
          [i, orderedIds[i]],
        );
      }
      await reloadCache();
