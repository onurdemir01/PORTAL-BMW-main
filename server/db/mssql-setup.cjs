...
const TABLES = [
  ...
  },
  {
    ...
    sql: `
      CREATE TABLE scalex_operations (
        id               INT IDENTITY(1,1) PRIMARY KEY,
  ...
  },
  {
    ...
    // `scalex-state-<app>` ConfigMap'idir; bu tablo "kim, ne zaman, hangi isle"
    // sorusunu cevaplar ve cluster gercegiyle ayrisirsa `drift_status` ile ISARETLENIR.
    // Ekran sapmayi GIZLEMEZ, gosterir.
    name: 'scalex_state_mirror',
    sql: `
      CREATE TABLE scalex_state_mirror (
        id                INT IDENTITY(1,1) PRIMARY KEY,
        env               NVARCHAR(50) NOT NULL,
        tenant            NVARCHAR(100) NOT NULL,
        cluster_name      NVARCHAR(200) NOT NULL,
  ...
  for (const { table, col, sql } of alters) {
    try {
      const has = await pool
        .request()
        .query(
          `SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='${table}' AND COLUMN_NAME='${col}' AND TABLE_SCHEMA IN ('dbo', SCHEMA_NAME())`,
        );
    ...
    try {
      const info = await pool.request().query(
        `SELECT CHARACTER_MAXIMUM_LENGTH AS len FROM INFORMATION_SCHEMA.COLUMNS
         WHERE TABLE_NAME='${table}' AND COLUMN_NAME='${col}' AND TABLE_SCHEMA IN ('dbo', SCHEMA_NAME())`,
      );
