...
// BICIM DEGIL KURAL. Prettier tek satirlik `if (...) { a; b; }` bloklarini
// satirlara aciyor ve tirnak stilini degistiriyor; asagidaki desenler bitisiklik
// varsaydigi icin kural aynen dururken kirmiziya donuyordu
// (bkz. bekci-korlugu-desenleri #2b).
const SRC = (RUNNER + '\n' + GATES).replace(/\s+/g, ' ').replace(/'/g, '"');
  ...
  (function walk(d) {
    for (const e of fs.readdirSync(d, { withFileTypes: true })) {
      ...
      const code = fs
        ...
        .filter((l) => !/^\s*(\/\/|\*)/.test(l))
        .join('\n');
      if (/require\([^)]*smart-gate\.cjs[^)]*\)/.test(code))
...
test('bekci KOR DEGIL: gercek bir kapi cagrisi silinirse kirmizi olur', () => {
  ...
  assert.ok(
    realCallSites(RUNNER).length >= 2,
    'baslangic durumu yanlis: runner.cjs 2 cagri tasimali',
  );
  ...
  assert.ok(
    realCallSites(sabotaged).length < 2,
...
test('gateVars yalnizca guvenilir anahtarlardan kurulur', () => {
  assert.match(
    SRC,
    /const gateVars = \{\};[\s\S]{0,200}if \(!untrustedKeys\.has\(k\)\) gateVars\[k\] = v;/,
  );
  assert.match(
    SRC,
