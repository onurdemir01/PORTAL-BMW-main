  ...
  return {
    ...
    artifacts: {
      scalex_result: {
        ...
        targets: [
          { cluster: 'c1', app: 'odeme-api', kind: 'deployment', status: 'OK', detail: '' },
        ],
      },
    },
  };
...
function withFakeDb(fn, { rows } = {}) {
  ...
  db.query = async (sql, params) => {
    ...
    if (
      /FROM scalex_operations\s+WHERE status = 'RUNNING'/.test(
        sql
          .replace(/\s+/g, ' ')
          .replace('FROM scalex_operations WHERE', 'FROM scalex_operations\n WHERE'),
      ) ||
...
function withFakeAwx(impl, fn) {
  ...
  return Promise.resolve(fn()).finally(() => {
    runner.getJobStatusOnServer = orig;
  });
}
...
test('sonucu kimse yoklamasa bile is SONUCLANDIRILIR (sekme kapatma senaryosu)', async () => {
  await withFakeDb(async (calls) => {
    ...
    const upd = calls.find(
      (c) => /UPDATE scalex_operations/.test(c.sql) && /status = 'FINISHED'/.test(c.sql),
    );
...
test('is HENUZ BITMEDIYSE hicbir sey yazilmaz', async () => {
  await withFakeDb(async (calls) => {
    await withFakeAwx(
      async () => ({ finished: false, status: 'running' }),
      async () => {
        const out = await reconciler.tick();
    ...
    assert.ok(
      !calls.some((c) => /UPDATE scalex_operations/.test(c.sql)),
      'calisan ise dokunulmamali',
    );
  });
});
...
test("yalnizca `apply` aynaya yazar — `dry_run` cluster'a dokunmadigi icin kayit URETMEZ", async () => {
  await withFakeDb(async (calls) => {
    ...
    const mirrorWrites = calls.filter(
      (c) => /scalex_state_mirror/.test(c.sql) && /^\s*(UPDATE|MERGE|DELETE|INSERT)\b/i.test(c.sql),
    );
    assert.deepEqual(
      mirrorWrites.map((c) => c.sql.trim().split('\n')[0]),
      [],
...
//
// Bu bekciler bir GERI ALMA KAYBINI kilitliyor. Kapi `pendingApproval` ile donunce
...
test('onay GELDIGINDE satir RUNNING olur ve gercek jobId yazilir', async () => {
  await withApprovalDb(
    async (calls) => {
      ...
      const upd = calls.find(
        (c) => /UPDATE scalex_operations/.test(c.sql) && /status = 'RUNNING'/.test(c.sql),
      );
      assert.ok(upd, "satir RUNNING'e cekilmeli — yoksa uzlastirici isi HIC gormez");
      assert.ok(
        /WHERE smart_ticket_id = \$1 AND status = 'PENDING_APPROVAL'/.test(upd.sql),
...
test('onay turu PATLASA BILE sonuclandirma turu calisir (iki tur bagimsiz)', async () => {
  const origGet = smartStore.getTicket;
  smartStore.getTicket = async () => {
    throw new Error('smart store coktu');
  };
  try {
    await withFakeDb(async (calls) => {
      await withFakeAwx(
  ...
  await withLockDb(
    async (calls) => {
      ...
      assert.equal(
        out.releasedLocks,
        1,
        'yetim kilit birakilmadi — uygulama bir daha geri alinamaz',
      );
      assert.ok(calls.some((c) => /SET phase = 'scaled_down'/.test(c.sql)));
    },
...
test('kilit: hic kilitli satir yoksa `scalex_operations` HIC sorgulanmaz', async () => {
  await withLockDb(
    async (calls) => {
      ...
      assert.ok(
        !calls.some((c) => /FROM scalex_operations/.test(c.sql)),
