// server/logx/v2/ocp-health.cjs — OCP katalogundaki bir cluster satirinin CANLI kontrolu:
...
//
// TOKENSIZ CALISIR ve bu BILEREK boyledir: portal katalogunda cluster token'i tutulmasi
...
// Pod durumu: salt-okunur `oc get`. Bastion cozumlemesi katalogun KENDI kapisindan
...
async function podStatus(id, { namespace = '', labelSelector = '' } = {}) {
  ...
  const { hosts, missing } = await adminData.resolveTerminalHosts(row.env, row.tenant, [
    row.cluster_name,
  ]);
  if (missing.length) {
    throw Object.assign(
  ...
  await require('../../ansible/template-preflight.cjs').assertTemplateAcceptsExtraVars(
    registryRow.awxServerId ?? 0,
    templateId,
    extraVars,
    { label: 'ocp_pod_status' },
  );
