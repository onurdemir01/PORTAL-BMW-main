...
test('legacy.transfer(): keşif sonucuyla eşleşmeyen bir path reddedilir, HİÇBİR job launch edilmez', async (t) => {
  const launchMock = t.mock.method(jobs, 'launchJob', async () => {
    throw new Error('launchJob ÇAĞRILMAMALIYDI');
  });
    ...
    hosts: [
      {
        host: 'GBJBOT22',
        status: 'ok',
        files: [{ path: '/vhosting8/APP-T.ear/logs/SystemOut.log' }],
      },
    ],
  };
  const requestRow = {
    request_id: 'req-1',
    discovery_result_json: JSON.stringify(discoveryResult),
  };
...
test('legacy.transfer(): keşif sonucuyla BİREBİR eşleşen path kabul edilir ve job doğru extra_vars ile launch edilir', async (t) => {
  ...
  const job = await legacy.transfer(requestRow, [
    { host: 'GBJBOT22', path: '/vhosting8/APP-T.ear/logs/SystemOut.log' },
  ]);

  assert.equal(job.id, 42);
  assert.equal(capturedArgs.requestId, 'req-3');
  assert.equal(capturedArgs.jobType, 'legacy_transfer');
  assert.deepEqual(capturedArgs.extraVars.selected_files, [
    { host: 'GBJBOT22', path: '/vhosting8/APP-T.ear/logs/SystemOut.log' },
  ]);
  assert.match(
    capturedArgs.extraVars.archive_name,
    /^[a-f0-9]{32}\.zip$/,
    'archive adı benzersiz/rastgele olmalı',
  );
});
...
test("restrictions.isAllowed(): Admin her zaman izinli, DB'ye hiç sorulmaz", async (t) => {
  const queryMock = t.mock.method(db, 'query', async () => {
    throw new Error('DB sorgusu ÇAĞRILMAMALIYDI');
  });
  const allowed = await restrictions.isAllowed('legacy_app', 'HERHANGI_BIR_APP', {
    username: 'admin1',
    role: 'Admin',
  });
...
test('restrictions.isAllowed(): kısıtlama satırı YOKSA varsayılan olarak herkese açık', async (t) => {
  ...
  const allowed = await restrictions.isAllowed('legacy_app', 'GBCEPPOSDASHBOARD', {
    username: 'user1',
    role: 'User',
  });
  assert.equal(allowed, true);
...
test('restrictions.isAllowed(): kısıtlama VARSA ve kullanıcı grant listesinde DEĞİLSE reddedilir', async (t) => {
  ...
  const allowed = await restrictions.isAllowed('legacy_app', 'GIZLI_APP', {
    username: 'user1',
    role: 'User',
  });
...
test('restrictions.assertAllowed(): reddedilince 403 fırlatır', async (t) => {
  ...
  await assert.rejects(
    () => restrictions.assertAllowed('ocp_namespace', 'x', { username: 'u', role: 'User' }),
    (err) => {
...
test("jobs.pollJob(): zaten terminal durumdaki job AWX'e tekrar sorulmaz (cache)", async (t) => {
  const statusMock = t.mock.method(runner, 'getJobStatusOnServer', async () => {
    throw new Error('ÇAĞRILMAMALIYDI');
  });
    ...
    rows: [
      {
        id: 1,
        request_id: 'r1',
        job_type: 'legacy_discovery',
        awx_server_id: 1,
        awx_job_id: 999,
        status: 'successful',
        artifacts_json: JSON.stringify({ overall_status: 'success', hosts: [] }),
...
test('jobs.pollJob(): job terminal ama artifacts.logx_result YOKSA hata mesajıyla işaretlenir (sessizce başarılı sayılmaz)', async (t) => {
  ...
  t.mock.method(db, 'query', async (sql, params) => {
    if (/UPDATE logx_v2_jobs/i.test(sql)) {
      ...
      return {
        rows: [{ id: 1, status: 'successful', artifacts_json: null, error_message: params[2] }],
      };
    }
    return { rows: [] };
  });
  ...
  // Kullaniciya giden metin SADE; playbook/set_stats teshisi artik teknik ayrintida.
  assert.match(capturedParams[2], /İş No: 999/, 'kullanıcı mesajı iş numarası taşımalı');
  assert.match(
    out.technicalDetail,
    /set_stats/,
    'teknik ayrıntı playbook yapılandırma sorununa işaret etmeli',
  );
});
...
test('downloads.issueDownloadToken(): token crypto-random 256-bit hex (64 karakter), NEWID()/UUID DEĞİL', async (t) => {
  ...
  const { token, expiresInMinutes } = await downloads.issueDownloadToken({
    ...
    username: 'user1',
    sessionToken: 'sess1',
    stagedPath: '/sw/BMW_PORTAL/logs/legacy/x.zip',
    filename: 'x.zip',
    sizeBytes: 100,
    isFallback: false,
  });
...
test("downloads.issueDownloadToken(): iki ardışık çağrı ASLA aynı token'ı üretmez", async (t) => {
  t.mock.method(db, 'query', async () => ({ rows: [] }));
  const a = await downloads.issueDownloadToken({
    requestId: 'r1',
    username: 'u',
    sessionToken: 's',
    stagedPath: '/x',
  ...
  const b = await downloads.issueDownloadToken({
    requestId: 'r1',
    username: 'u',
    sessionToken: 's',
    stagedPath: '/x',
...
test("legacy.searchApps(): Envanter DB erişilemezse son bilinen snapshot'a otomatik düşer", async (t) => {
  ...
  fs.writeFileSync(
    snapshotPath,
    JSON.stringify({
      apps: ['GBCEPPOSDASHBOARD', 'GBSVCCOMPASS'],
      appHosts: { GBCEPPOSDASHBOARD: ['GBJBOT22'] },
      generatedAt: new Date().toISOString(),
    }),
  );

  t.mock.method(inventoryDb, 'getPool', async () => {
    throw new Error('MSSQL bağlantısı yok (simüle DB kesintisi).');
  });
...
test('ocp.selectClusters(): hicbir jump server cozulemezse job hic launch edilmeden reddedilir', async (t) => {
  ...
  t.mock.method(admin, 'resolveTerminalHosts', async (env, tenant, names) => ({
    hosts: {},
    missing: [...names],
  }));
  const requestRow = { request_id: 'r1' };
  await assert.rejects(
    ...
    (err) => {
      ...
      assert.match(
        err.message,
        /gbocptest1/,
        "hangi cluster(lar)in bastion'siz oldugu mesajda gecmeli",
      );
      return true;
    },
  );
...
test('requests.expireOldRequests(): 3 suresi dolmus istek icin dosya silme per-row, state UPDATE TEK sorgu', async (t) => {
  ...
  const deleteCalls = [];
  const count = await requests.expireOldRequests(async (p) => {
    deleteCalls.push(p);
  });
...
test("downloads.cleanupExpiredDownloads(): SELECT ile gorulen token'lar DELETE...IN(...) ile hedeflenir", async (t) => {
  const queryMock = t.mock.method(db, 'query', async (sql) => {
    if (/^SELECT/.test(sql)) {
      return {
        rows: [
          { token: 'tok1', staged_path: '/staging/x.zip' },
          { token: 'tok2', staged_path: null },
        ],
      };
    }
    return { rowCount: 2 };
...
test('legacy.transfer(): 0 byte dosya reddedilir, HICBIR job launch edilmez', async (t) => {
  ...
  await assert.rejects(
    () =>
      legacy.transfer(requestRow, [
        { host: 'GBJBOT22', path: '/vhosting8/APP-T.ear/logs/bos.log' },
      ]),
    (err) => {
      assert.equal(err.status, 400);
...
test('legacy.transfer(): boyutu BILINMEYEN dosya engellenmez (eski kesif ciktisi kirilmaz)', async (t) => {
  ...
  let launched = 0;
  t.mock.method(jobs, 'launchJob', async () => {
    launched += 1;
    return { id: 1 };
  });
  ...
  const discoveryResult = {
    overall_status: 'success',
    hosts: [
      { host: 'GBJBOT22', status: 'ok', files: [{ path: '/vhosting/APP.ear/logs/eski.log' }] },
    ],
  };
  const requestRow = {
  ...
  await legacy.transfer(requestRow, [
    { host: 'GBJBOT22', path: '/vhosting/APP.ear/logs/eski.log' },
  ]);
