...
// ayni namespace'e her giriste (ve HER kullanici icin) yeniden ~1 dk'lik AWX job'i
...
function withDb(handler, fn) {
  ...
  db.query = async (sql, params) => {
    ...
    if (/GETUTCDATE\(\) AS now_utc/i.test(sql))
      return { rows: [{ now_utc: '2026-01-01T00:00:00.000Z' }] };
    return handler(sql, params) ?? { rows: [], rowCount: 0 };
  };
  return Promise.resolve(fn(calls)).finally(() => {
    db.query = origQuery;
    cfgMod.getConfig = origCfg;
  });
}
  ...
  await withDb(
    ...
    async () => {
      ...
      assert.equal(
        out.cached,
        false,
        'cached sozlesmesi DEGISMEMELI (mevcut akislar buna bakiyor)',
      );
...
test('getApps(): tarama kaydi tablosu YOKSA (eski kurulum) akis durmaz', async () => {
  await withDb(
    (sql) => {
      if (/FROM ocp_app_scan_log/i.test(sql))
        throw new Error("Invalid object name 'ocp_app_scan_log'.");
      if (/FROM ocp_app_cache/i.test(sql))
        return {
          rows: [
            {
              kind: 'Deployment',
