...
async function finalizeIfNeeded(requestRow, jobBefore, jobAfter) {
  if (
    jobsMod.TERMINAL_STATUSES.has(jobBefore.status) ||
    !jobsMod.TERMINAL_STATUSES.has(jobAfter.status)
  ) {
  ...
  switch (jobAfter.jobType) {
    ...
    case 'ocp_discover_fetch': {
      ...
      const files =
        Array.isArray(art.staged_files) && art.staged_files.length
          ? art.staged_files.filter((f) => f && f.staged_path)
          : art.staged_path
            ? [
                {
                  staged_path: art.staged_path,
                  filename: art.filename,
                  size_bytes: art.size_bytes,
                  is_fallback: art.is_fallback,
                },
              ]
            : [];
      if (files.length) {
        ...
        const already = await downloads
          .listTokenizedPaths(requestRow.request_id)
          .catch(() => new Set());
        const prefixes = [];
        const failures = [];
        ...
        await audit
          .log({
            username: requestRow.username,
            action:
              jobAfter.jobType === 'legacy_transfer' ? 'v2_transfer' : 'v2_ocp_discover_fetch',
            result: art.overall_status || 'unknown',
            detail:
              `download_token_issued count=${prefixes.length}/${files.length} token_prefixes=${prefixes.join(',')}` +
        ...
        await requests.updateRequest(requestRow.request_id, {
          state: 'failed',
          errorMessage: jobAfter.errorMessage || 'Transfer başarısız oldu.',
        });
      }
      break;
    }
...
function initLogXv2(app) {
  ...
  try {
    router.use('/admin', require('../../audit/index.cjs').auditMutations('logx_admin'));
  } catch {
    /* yoksay */
  }
  ...
  async function filterDiscoveryResult(request, user) {
    ...
    for (const c of result.clusters) {
      if (!Array.isArray(c?.namespaces)) {
        clusters.push(c);
        continue;
      }
      const prefix = `${input.tenant}/${input.env}/${c.cluster_name}/`;
      const allowed = new Set(
        await restrictions.filterAllowed(
          'ocp_namespace',
          c.namespaces.map((n) => prefix + n),
          user,
        ),
      );
    ...
    asyncRoute(async (req, res) => {
      ...
      const download =
        request.state === 'ready'
          ...
          : null;
      const downloadList =
        request.state === 'ready' ? await downloads.listDownloadsForRequest(row.request_id) : [];
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      if (!patch)
        return res.status(400).json({ ok: false, message: 'Geçersiz geri-dönüş hedefi.' });
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      if (app) {
        // ELLE GIRILEN UYGULAMA ADI DA BU KAPIDAN GECER. Kisitlama modeli
        // VARSAYILAN-ACIK oldugu icin (satiri olmayan kaynak herkese acik) bilinmeyen
        // bir ad zaten gecerdi; ama bir kisit TANIMLIYSA elle yazmak onu ATLATAMAZ.
        await restrictions.assertAllowed('legacy_app', app, currentUser(req));
      }
      ...
      if (manualHosts.length) {
        await audit
          .log({
            ...
            result: 'accepted',
            detail: `app=${app} hosts=${manualHosts.join(',')} jobId=${job.id}`,
          })
          .catch(() => {});
      }
  ...
  async function assertNamespaceAllowed(input, namespace, user) {
    ...
    for (const cluster of clusters) {
      await restrictions.assertAllowed(
        'ocp_namespace',
        `${input.tenant}/${input.env}/${cluster}/${namespace}`,
        user,
      );
    }
  }
  ...
  router.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      const out = await require('./ocp-cache.cjs').getNamespaces({
        env,
        tenant,
        clusterName: cluster,
      });
  ...
  router.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      const out = await require('./ocp-catalog.cjs').getNamespaces({
        env,
        tenant,
      ...
      for (const clusterName of clusters) {
        ...
        for (const k of await restrictions.filterAllowed(
          'ocp_namespace',
          out.items.map((ns) => prefix + ns),
          currentUser(req),
        )) {
  ...
  router.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      const elapsedSec = after.startedAt
        ? Math.floor((Date.now() - new Date(after.startedAt).getTime()) / 1000)
        : 0;
    ...
    asyncRoute(async (req, res) => {
      ...
      await audit
        .log({
          ...
          action: 'v2_cancel',
          result: after.status,
          detail: `jobType=${after.jobType} jobId=${after.id}`,
        })
        .catch(() => {});
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      res.json(
        await require('./ocp-health.cjs').podStatus(req.params.id, { namespace, labelSelector }),
      );
    }),
  );
  ...
  router.put(
    ...
    asyncRoute(async (req, res) => {
      res.json({
        ok: true,
        config: await require('./ocp-runtime-config.cjs').saveConfig(req.body || {}),
      });
    }),
  );
  ...
  router.put(
    '/admin/ocp-terminal-host-map/:id',
    requireAdmin,
    asyncRoute(async (req, res) => {
  ...
  try {
    require('./ocp-sync.cjs').startOcpSync();
  } catch (e) {
