...
const STATUS: Record<
  NginxMigrationApp['status'],
  { label: string; tone: 'success' | 'warning' | 'danger' | 'neutral'; hint: string }
> = {
  ready: {
    label: 'hazır',
    tone: 'success',
    hint: 'Yeni sunucuların HEPSİNDE hysdeploy + applications dizini var',
  },
  partial: {
    label: 'kısmi',
    tone: 'warning',
    hint: 'Bazı yeni sunucularda var, bazılarında yok (ya da bazıları henüz taranmadı)',
  },
  missing: {
    label: 'EKSİK',
    tone: 'danger',
    hint: 'Taranan hiçbir yeni sunucuda yok — taşıma öncesi deploy gerekli',
  },
  'not-scanned': {
    label: 'taranmadı',
    tone: 'neutral',
    hint: 'Yeni sunucuların hiçbiri henüz taranmadı (nginx_config_audit job’ı koşmalı)',
  },
};
...
export default function NginxProdMigration() {
  ...
  const [config, setConfig] = useState<NginxMigrationConfig>({
    awxServerId: 0,
    templateId: 0,
    deleteTemplateId: 0,
  });
  ...
  async function confirmDelete() {
    ...
    try {
      const r = await nginxMigrationApi.remove({
        group: pendingDelete.group.id,
        namespace: pendingDelete.app.namespace,
        application: pendingDelete.app.application,
        service: path.service,
        inputPath: path.location,
      });
      if (r.ok) {
  ...
  return (
    <div className="space-y-3">
      ...
      {result && (
        <Note
          tone={result.tone === 'ok' ? 'success' : 'danger'}
          title={result.tone === 'ok' ? 'İş başlatıldı' : 'İş başlatılamadı'}
        >
          {result.text}
        </Note>
      )}
      ...
      {!configured && (
        <Note tone="warning" title='"Tanım oluştur" henüz çalışmıyor — yapılandırma eksik'>
          <Code>bmw_nginx/nginx_ops/nginx_prod_migration.yml</Code> için AWX&apos;te bir job
          template açılıp Portal&apos;a tanıtılması gerekiyor. Bu yapılana kadar düğme açık bir hata
          döndürür.
          {!isAdmin && (
            <div className="mt-1">
      ...
      {!data.proxyReady && (
        <Note tone="warning" title="Eski sunucu proxy verisi yok">
          <Code>dbo.Nginx_Config_Audit</Code>&apos;te proxy satırı yok (DDL{' '}
          <Code>nginx_config_audit_migrate_proxy.sql</Code> ya da job henüz koşmamış).
        </Note>
      )}
      ...
      <div className="flex items-center gap-2 justify-end flex-wrap">
        ...
        <select
          value={trackFilter}
          onChange={(e) =>
            setTrackFilter(e.target.value as 'all' | 'open' | 'planned' | 'migrated')
          }
          className="px-2 py-1.5 text-xs border border-[var(--border)] rounded-lg bg-[var(--bg-surface)]"
          ...
          onClick={() =>
            csvDownload(
              ...
              [
                ...
                'eski_locationlar',
                'hedef',
                ...data.groups.flatMap((g) => g.newHosts),
              ],
              data.groups.flatMap((g) =>
                g.apps.map((a) => [
                  ...
                  a.owner?.groups.join(' | ') || '',
                  TRACK_LABEL[
                    tracking.get(trackKey(g.id, a.namespace, a.application))?.state || 'none'
                  ].label,
                  ...
                  a.services.join(' '),
                  a.locationCount,
                  a.paths.map((p) => p.service + ' ' + p.location).join(' | '),
                  a.target,
                  ...data.groups.flatMap((gg) =>
                    gg.newHosts.map((h) => (gg.id !== g.id ? '' : cellText(a.perHost[h]))),
                  ),
                ]),
              ),
            )
      ...
      <div className="flex gap-1 flex-wrap border-b border-[var(--border)]">
        {data.groups.map((g) => {
          ...
          return (
            ...
            >
              {g.label}
              <span className="ml-1.5 text-[10px] font-normal tabular-nums text-[var(--text-muted)]">
                {nf(loc.defined)}/{nf(loc.total)}
              </span>
            </button>
          );
        })}
      ...
      {data.groups
        .filter((g) => g.id === (groupId || data.groups[0]?.id))
        .map((g) => (
          <GroupPanel
            key={g.id}
            g={g}
            ...
            return (
              <div className="space-y-2 text-[12px]" style={{ color: 'var(--text-secondary)' }}>
                {pending.app.paths.length > 1 && (
                  <div>
                    Bu uygulama eski sunucuda <b>{pending.app.paths.length}</b> farklı
                    location&apos;dan sunuluyor; hangisi taşınsın?
                      ...
                      {pending.app.paths.map((p, i) => (
                        ...
                        >
                          <input
                            type="radio"
                            name="path"
                            checked={i === pending.pathIdx}
                            onChange={() => setPending({ ...pending, pathIdx: i })}
                          />
                          <span
                            className="font-mono text-[11px]"
                            style={{ color: 'var(--text-primary)' }}
                          >
                            {p.service}-PROD.conf · {p.location}
                          </span>
                ...
                <p>
                  Yeni sunucularda (
                  <span className="font-mono">{pending.group.newHosts.join(', ')}</span>) şunlar
                  oluşturulacak:
                </p>
                <div
                  className="font-mono text-[11px] space-y-0.5 rounded-lg px-3 py-2"
                  style={{ background: 'var(--bg-elevated)', color: 'var(--text-primary)' }}
                >
                  <div>
                    /usr/nginx/conf.d/{svc}-PROD.conf →{' '}
                    <span className="text-[var(--text-muted)]">
                      location {path.location} {'{'} include …/{svc.toLowerCase()}-
                      {pending.app.application}-{pending.app.namespace}.conf; {'}'}
                    </span>
                  </div>
                  <div>
                    /usr/nginx/conf.d/application-confs/{svc.toLowerCase()}-
                    {pending.app.application}-{pending.app.namespace}.conf
                  </div>
                </div>
                <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>
                  Non-prod SPA oluşturma akışının aynısı: uygulama dizini yoksa durur; aynı context
                  path zaten tanımlıysa durur; aynı ad varsa dosya <Code>-N</Code> eki alır;{' '}
                  <Code>nginx -t</Code> düşerse değişiklik geri alınır, geçerse reload. Eski
                  sunucudaki proxy_pass tanımına <b>dokunulmaz</b>; trafik geçişi ayrıca planlanır.
                </p>
              </div>
            );
          })()}
      ...
      <Modal
        ...
        subtitle={
          pendingDelete
            ? `${pendingDelete.app.application} · ${pendingDelete.app.namespace}`
            : undefined
        }
        icon={TrashIcon}
        dismissOnBackdrop={false}
      ...
      >
        {pendingDelete &&
          (() => {
            const path = pendingDelete.app.paths[pendingDelete.pathIdx];
            const t = tracking.get(
              trackKey(
                pendingDelete.group.id,
                pendingDelete.app.namespace,
                pendingDelete.app.application,
              ),
            );
            return (
              <div className="space-y-2 text-[12px]" style={{ color: 'var(--text-secondary)' }}>
                {pendingDelete.app.paths.length > 1 && (
                  <div>
                    Bu uygulama eski sunucuda <b>{pendingDelete.app.paths.length}</b> location'dan
                    sunuluyor; hangisi kaldırılsın?
                    <div className="mt-1 space-y-0.5">
                      {pendingDelete.app.paths.map((p, i) => (
                        ...
                        >
                          <input
                            type="radio"
                            name="delpath"
                            checked={i === pendingDelete.pathIdx}
                            onChange={() => setPendingDelete({ ...pendingDelete, pathIdx: i })}
                          />
                          <span
                            className="font-mono text-[11px]"
                            style={{ color: 'var(--text-primary)' }}
                ...
                <p>
                  Eski sunucularda (
                  <span className="font-mono">{pendingDelete.group.oldHosts.join(', ')}</span>)
                  şunlar kaldırılacak:
                </p>
                <div
                  className="font-mono text-[11px] space-y-0.5 rounded-lg px-3 py-2"
                ...
                >
                  <div>
                    /usr/nginx/conf.d/{path.service}-PROD.conf →{' '}
                    <span className="text-[var(--text-muted)]">
                      location {path.location} {'{ proxy_pass … }'}
                    </span>
                  </div>
                  <div>
                    upstream {pendingDelete.app.written.join(' / ')}{' '}
                    <span className="text-[var(--text-muted)]">
                      — yalnızca başka hiçbir location kullanmıyorsa
                    </span>
                  </div>
                </div>
                {t?.state !== 'migrated' && (
                  <Note tone="warning" title="Geçiş kaydı 'geçti' değil">
                    Bu uygulamanın geçiş takibi <b>{t ? TRACK_LABEL[t.state].label : 'kayıtsız'}</b>
                    . Trafik yeni sunuculara alınmadan eski tanım kaldırılırsa uygulama{' '}
                    <b>erişilemez</b> olur. Emin değilseniz iptal edin.
                  </Note>
                )}
                <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>
      ...
      {editing && (
        <TrackingModal
          ...
          current={
            tracking.get(
              trackKey(editing.group.id, editing.app.namespace, editing.app.application),
            ) || null
          }
          onClose={() => setEditing(null)}
          onSaved={(row) => {
            setTracking((m) =>
              new Map(m).set(trackKey(row.group, row.namespace, row.application), row),
            );
            setEditing(null);
          }}
        />
...
// DUZ TARIH (YYYY-MM-DD) — ortak `fmtDate` DEGIL, bilerek.
// Bu degerler DB'den saatsiz gelir; ortak yardimci onlari Date'e cevirip saat
// dilimine gore bicimler ve bos degerde '—' doner. Burada istenen sey yalniz
// gun.ay.yil ve bos deger icin BOS metin. Ham `toLocale*` kullanmadigi icin
// G19 bekcisini de ihlal etmez.
...
function TrackCell({
  ...
  const title = [
    ...
    t?.note ? `not: ${t.note}` : '',
    t?.configJobId
      ? `tanım job ${t.configJobId} (${t.configCreatedAt ? fmtDateTime(t.configCreatedAt) : ''}${t.configCreatedBy ? ', ' + t.configCreatedBy : ''})`
      : '',
    t?.deleteJobId
      ? `eski tanım kaldırma job ${t.deleteJobId} (${t.deleteRequestedAt ? fmtDateTime(t.deleteRequestedAt) : ''}${t.deleteRequestedBy ? ', ' + t.deleteRequestedBy : ''}) — 23:00'e zamanlandı`
      : '',
  ...
  return (
    <button
      ...
      title={ready ? title : 'takip tablosu okunamadı'}
    >
      <Pill tone={meta.tone}>
        {meta.label}
        {date ? ` ${fmtPlainDate(date)}` : ''}
      </Pill>
      {t?.configJobId && (
        <span
          className="text-[9px] text-[var(--text-muted)]"
          title="Tanım oluştur job'ı koşturuldu"
        >
          ⚙{t.configJobId}
        </span>
      )}
      {t?.deleteJobId && (
        <span
          ...
          title="Eski tanımı kaldırma job'ı koşturuldu (23:00'e zamanlandı)"
        >
          🗑{t.deleteJobId}
        </span>
      )}
...
function TrackingModal({
  ...
  group: NginxMigrationGroup;
  app: NginxMigrationApp;
  current: MigrationTracking | null;
  onClose: () => void;
  onSaved: (row: MigrationTracking) => void;
}) {
    ...
    >
      <div className="space-y-3 text-[12px]" style={{ color: 'var(--text-secondary)' }}>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <label className="flex flex-col gap-1">
            <span className="text-[10px] uppercase tracking-wide text-[var(--text-muted)]">
              Geçiş tarihi{state === 'migrated' ? ' *' : ''}
            </span>
            <input
              type="date"
              value={migratedDate}
              onChange={(e) => setMigratedDate(e.target.value)}
              className={inputCls}
            />
          </label>
        </div>
        ...
        {current?.configJobId && (
          <div className="text-[11px] text-[var(--text-muted)]">
            Tanım oluştur job&apos;ı: <b>{current.configJobId}</b>
            {current.configCreatedAt ? ` · ${fmtDateTime(current.configCreatedAt)}` : ''}
            {current.configCreatedBy ? ` · ${current.configCreatedBy}` : ''}
          </div>
        )}
        {current?.updatedBy && (
          <div className="text-[11px] text-[var(--text-muted)]">
            Son güncelleme: {current.updatedBy}
            {current.updatedAt ? ` · ${fmtDateTime(current.updatedAt)}` : ''}
          </div>
        )}
...
function MigrationConfigPanel({
  config,
  onSaved,
}: {
  config: NginxMigrationConfig;
  onSaved: () => void;
}) {
  ...
  // PROP DEGISINCE STATE'I AYARLA — EFFECT DEGIL, RENDER SIRASINDA.
  //
  // Effect'te yapmak, ESKI degerlerle BIR RENDER daha uretiyordu (kullanici bir an
  // onceki job kimligini goruyordu) ve React 19 bunu `set-state-in-effect` ile
  // isaretliyordu. Bu, React'in belgeledigi "prop degisince state'i ayarla" deseni:
  // kosul bir sonraki render'da yanlis olur, yani yakinsar.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive)
          setMsg({ tone: 'bad', text: 'AWX sunucu listesi alınamadı; ID elle girilebilir.' });
      }
    })();
    return () => {
      alive = false;
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      const r = await nginxMigrationApi.saveConfig({
        awxServerId,
        templateId: tid,
        deleteTemplateId: Number(deleteTemplateId) || 0,
      });
      if (r.ok) {
        setMsg({ tone: 'ok', text: 'Kaydedildi. Düğme artık çalışır.' });
        onSaved();
  ...
  return (
    <Panel
      title="Job yapılandırması (yönetici)"
      description="Tanım oluştur: nginx_ops/nginx_prod_migration.yml template'i · Eski tanımı kaldır: mevcut nginx_ops (Nginx Reverse Proxy Operations, survey'li) template'i — action=delete, env=prod ile koşar, 23:00'e zamanlar"
      dense
    >
      <div className="flex flex-wrap items-end gap-3 px-3 py-2">
        ...
        <label className="flex flex-col gap-1">
          <span className="text-[10px] uppercase tracking-wide text-[var(--text-muted)]">
            Silme job'ı (nginx_ops) ID
          </span>
          <input
            value={deleteTemplateId}
            onChange={(e) => setDeleteTemplateId(e.target.value.replace(/[^0-9]/g, ''))}
        ...
        {msg && (
          <span
            className={`text-[11px] ${msg.tone === 'ok' ? 'text-emerald-700' : 'text-red-600'}`}
          >
            {msg.text}
          </span>
        )}
...
function FormCell({
  ...
  const title = [
    `yazılan: ${written.join(', ')}`,
    `gerçek hedef: ${target}`,
    `kaynak: ${SOURCE_LABEL[source] || source}`,
  ].join('\n');
  return (
...
const STATUS_ORDER: Record<NginxMigrationApp['status'], number> = {
  missing: 0,
  partial: 1,
  'not-scanned': 2,
  ready: 3,
...
const NEW_LOC: Record<
  'defined' | 'partial' | 'none' | 'not-scanned',
  { mark: string; color: string; hint: string }
> = {
...
function LocationProgress({ g }: { g: NginxMigrationGroup }) {
  ...
  const seg = (n: number, color: string, label: string) =>
    n > 0 ? (
      <span
        className="h-full"
        style={{ width: `${p(n)}%`, background: color }}
        title={`${label}: ${nf(n)}`}
      />
    ) : null;
  return (
    ...
    >
      <div className="flex items-baseline justify-between gap-3 flex-wrap">
        <div>
          ...
          >
            Taşıma ilerlemesi — location bazında
          </div>
          <div className="text-sm" style={{ color: 'var(--text-primary)' }}>
            <b className="text-lg tabular-nums">{nf(t.defined)}</b> / {nf(t.total)} location yeni
            sunucularda tanımlı
            <span
          ...
          <tbody>
            {g.serviceLocations.map((x) => {
              ...
              return (
                ...
                >
                  <td className="pr-4 py-0.5 font-mono" style={{ color: 'var(--text-primary)' }}>
                    {x.service}-PROD.conf
                  </td>
                  <td className="pr-4 py-0.5 text-right tabular-nums">{nf(x.locations)}</td>
                  <td
                    className="pr-4 py-0.5 text-right tabular-nums"
                  ...
                  <td
                    className="pr-4 py-0.5 text-right tabular-nums"
                    style={{ color: x.partial ? 'var(--status-warning)' : undefined }}
                  >
                    {nf(x.partial)}
                  </td>
                  <td
                    className="pr-4 py-0.5 text-right tabular-nums"
                    style={{ color: x.none ? 'var(--status-danger)' : undefined }}
                  >
                    {nf(x.none)}
                  </td>
      ...
      <div className="text-[10px]" style={{ color: 'var(--text-muted)' }}>
        Sayım vhost başına farklı location; SPA olmayan (API) ve çözülemeyen hedefler de dâhil —
        vhost'un tamamı. “Tanımlı” = yeni sunucunun <Code>{'<SERVIS>'}-PROD.conf</Code> dosyasında
        aynı location var (include ya da proxy). Uygulama satırlarındaki H/A/C ise dosyaların hazır
...
function GroupPanel({
  ...
  const trackOf = (a: NginxMigrationApp) =>
    tracking.get(trackKey(g.id, a.namespace, a.application)) || null;
  const rows = useMemo(() => {
    ...
    let list = g.apps.filter((a) => {
      ...
      if (
        needle &&
        !a.application.includes(needle) &&
        !a.namespace.includes(needle) &&
        !ownerText(a.owner).includes(needle)
      )
        return false;
      return true;
    });
    if (sortBy === 'team') {
      ...
      const d = (a: NginxMigrationApp) => {
        const t = trackOf(a);
        return t?.migratedDate || t?.plannedDate || '';
      };
      list = [...list].sort((a, b) => {
        const da = d(a),
          db = d(b);
        if (!da !== !db) return da ? -1 : 1;
        return da.localeCompare(db) || a.application.localeCompare(b.application);
      });
    }
    return list;
  ...
  const teamSummary = useMemo(() => {
    ...
    for (const a of g.apps) {
      const t = a.owner?.groups[0] || '(ekip bilinmiyor)';
      m.set(t, (m.get(t) || 0) + 1);
    }
      ...
      description={
        ...
        <span className="block space-y-0.5">
          <span className="block">
            <span className="inline-block w-9 font-semibold">eski</span>
            <span className="font-mono">{g.oldHosts.join(', ')}</span>
          </span>
          <span className="block">
            <span className="inline-block w-9 font-semibold">yeni</span>
    ...
    >
      <div className="p-3 space-y-3">
        ...
        <div className="grid gap-2 sm:grid-cols-3 lg:grid-cols-6">
          <StatTile
            label="uygulama"
            value={nf(g.totals.apps)}
            hint="eski sunuculardaki SPA hedefleri"
          />
          <StatTile
            label="hazır"
            value={nf(g.totals.ready)}
            tone={g.totals.ready === g.totals.apps && g.totals.apps > 0 ? 'success' : 'neutral'}
            hint={STATUS.ready.hint}
          />
          <StatTile
            label="kısmi"
            value={nf(g.totals.partial)}
            tone={g.totals.partial ? 'warning' : 'neutral'}
            hint={STATUS.partial.hint}
          />
          <StatTile
            label="eksik"
            value={nf(g.totals.missing)}
            tone={g.totals.missing ? 'danger' : 'neutral'}
            hint={STATUS.missing.hint}
          />
          <StatTile
            label="SPA değil"
            value={nf(g.totals.nonSpa)}
            hint="API/arka uç hedefi — dizin beklenmez"
          />
          <StatTile
            label="çözülemedi"
            value={nf(g.totals.unresolved)}
            tone={g.totals.unresolved ? 'warning' : 'neutral'}
            hint="hedef (ns, app)'a eşlenemedi — aşağıda"
          />
        </div>
        <div className="grid gap-2 sm:grid-cols-3">
          <StatTile
            label="geçti"
            value={nf(trackTotals.migrated)}
            tone={trackTotals.migrated ? 'success' : 'neutral'}
            hint={`${g.totals.apps} uygulamanın ${trackTotals.migrated} tanesi yeni sunuculara geçti (takip kaydı)`}
          />
          <StatTile
            label="planlandı"
            value={nf(trackTotals.planned)}
            tone={trackTotals.planned ? 'warning' : 'neutral'}
            hint="geçiş tarihi belirlenmiş, henüz geçmedi"
          />
          <StatTile
            label="henüz planı yok"
            value={nf(trackTotals.none)}
            hint={`${trackTotals.cancelled} iptal · Geçiş sütunundaki rozete tıklayarak planlayın`}
          />
        </div>

        {notScanned.length > 0 && (
          <Note tone="warning" title={`${notScanned.length} yeni sunucu henüz taranmadı`}>
            <span className="font-mono">{notScanned.join(', ')}</span> için dizin verisi yok; bu
            sütunlar <i>taranmadı</i> görünür ve hiçbir uygulama &quot;hazır&quot; sayılmaz.{' '}
            <Code>nginx_config_audit</Code> job&apos;ının bu sunucuları kapsayan sürümü koşmalı
            (Ansible 13f48b054).
          </Note>
        )}
        {oldNotSeen.length > 0 && g.oldHostsSeen.length > 0 && (
          <div className="text-[11px] text-[var(--text-muted)]">
            Eski sunuculardan tarama verisi olmayanlar:{' '}
            <span className="font-mono">{oldNotSeen.join(', ')}</span>
          </div>
        )}
        {g.oldHostsSeen.length === 0 && (
          <Note tone="warning" title="Eski sunuculardan proxy kaydı yok">
            Bu gruptaki hiçbir eski sunucu için proxy satırı bulunamadı; karşılaştırılacak uygulama
            listesi boş.
          </Note>
        )}
          ...
          <table className="text-[11px] w-full">
            <thead>
              <tr className="text-[var(--text-muted)]">
                ...
                <th className="text-left pr-3 pb-1" title="namespace'in CMDB sahibi">
                  Ekip
                </th>
                <th
                  className="text-left pr-3 pb-1"
                  title="geçiş takibi: planlandı / geçti / iptal + tarih; tıklayarak düzenleyin"
                >
                  Geçiş
                </th>
                ...
                <th
                  className="text-left pr-3 pb-1"
                  title="eski sunucudaki vhost ve location tanımları (proxy_pass ile). Sondaki işaret: bu location YENİ sunucularda tanımlı mı (✓ hepsinde, ◐ bazısında, ✗ hiçbirinde, ? taranmadı)"
                >
                  Location (eski → yeni)
                </th>
                <th
                  className="text-left pr-3 pb-1"
                  title="proxy_pass yazımı: FQDN ya da upstream adı; ipucunda yazılan ad(lar) ve gerçek hedefin kaynağı"
                >
                  Yazım
                </th>
                {g.newHosts.map((h) => (
                  <th
                    key={h}
                    className="text-center px-1.5 pb-1 font-mono whitespace-nowrap"
                    title={g.newHostsScanned.includes(h) ? 'tarandı' : 'henüz taranmadı'}
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
                ...
                >
                  <td className="pr-2 py-1">
                    <button
                      ...
                      title={
                        !canCreate
                          ? 'Job yapılandırılmamış (yönetici paneli)'
                          : a.status === 'missing'
                            ? 'Taranan hiçbir yeni sunucuda uygulama dizini yok — önce deploy'
                            : a.status === 'not-scanned'
                  ...
                  <td
                    className="pr-3 py-1 font-mono whitespace-nowrap"
                    title={`hedef: ${a.target} · çözüm: ${a.how === 'route' ? 'route adresi (kesin)' : 'OpenShift envanter çifti'}`}
                  >
                    {a.application}
                  </td>
                  <td className="pr-3 py-1 font-mono text-[var(--text-muted)] whitespace-nowrap">
                    ...
                    {a.suffixAdded && (
                      <span
                        className="ml-1 text-[9px] px-1 rounded border"
                        style={{ borderColor: 'var(--border-subtle)', color: 'var(--text-muted)' }}
                        title="proxy_pass yazımında -prod yoktu; eklenerek eşlendi"
                      >
                        +prod
                      </span>
                    )}
                  ...
                  <td className="pr-3 py-1">
                    <OwnerCell owner={a.owner} ready={ownersReady} />
                  </td>
                  <td className="pr-3 py-1">
                    <TrackCell t={trackOf(a)} ready={trackingReady} onEdit={() => onTrack(a)} />
                  </td>
                  <td className="pr-3 py-1">
                    <Pill tone={STATUS[a.status].tone} title={STATUS[a.status].hint}>
                      {STATUS[a.status].label}{' '}
                      {a.status !== 'not-scanned' && `${a.readyHosts}/${g.newHosts.length}`}
                    </Pill>
                  </td>
                      ...
                      {a.paths.map((p) => (
                        ...
                        >
                          ...
                          <span
                            ...
                            style={{ color: NEW_LOC[p.newStatus].color }}
                          >
                            {NEW_LOC[p.newStatus].mark}
                          </span>
                        </span>
                      ))}
                    </div>
                    <div className="text-[10px] text-[var(--text-muted)] mt-0.5">
                      {a.oldHosts.length} sunucu
                    </div>
                  </td>
              ...
              {rows.length === 0 && (
                <tr>
                  <td colSpan={8 + g.newHosts.length} className="py-2 text-[var(--text-muted)]">
...
function OtherList({
  ...
  title: string;
  hint: string;
  rows: NginxMigrationOther[];
  tone: 'warning' | 'neutral';
}) {
  return (
    <details>
      <summary
        className={`text-[11px] cursor-pointer select-none ${tone === 'warning' ? 'text-amber-700' : 'text-[var(--text-muted)]'}`}
      >
        {title} — {hint}
      </summary>
