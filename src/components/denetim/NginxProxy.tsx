  ...
  if (!data.schemaReady) {
    return (
      <Note tone="warning" title="Proxy denetimi henüz açık değil">
        ...
        Çalıştırıldıktan sonra <Code>nginx_config_audit</Code> job&apos;ının bir sonraki
        koşusunda burası dolar. <b>Mevcut SPA ekranları bu adımdan etkilenmez.</b>
      </Note>
    );
  }
  ...
  if (!data.scanDate) {
    return (
      <Note tone="info" title="Henüz proxy kaydı yok">
        Şema hazır ancak <Code>nginx_config_audit</Code> job&apos;ı DDL sonrası henüz
        çalışmamış olabilir. Bir sonraki taramadan sonra burası dolar.
      </Note>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <Note tone="info" title="Bu görünüm ne gösteriyor?">
        ...
        <Code>location … &#123; proxy_pass https://&lt;upstream&gt;/; &#125;</Code> şeklindedir.
        Her satır bir tanımdır ve prod&apos;da aynı tanım birden çok sunucuda aynadır —{" "}
        <b>Sunucu</b> sütunu kaçında görüldüğünü söyler, eksik kalan bir sunucu buradan
        fark edilir. SPA&apos;ya özgü alanlar (include dosyası, deploy modu, paket var mı)
        proxy tanımında anlamsız olduğu için gösterilmez.
      </Note>
            ...
            <button
              onClick={() =>
                csvDownload(
                  ...
                  ["servis", "ortam", "vhost", "path", "upstream", "hedef_adres", "upstream_tanimli", "sunucular", "durum"],
                  rows.map((r) => [
                    r.service, r.env, r.vhost, r.locationPath, r.upstreamName || "",
                    r.targetUrl || "", r.upstreamDefined ? "EVET" : "HAYIR",
                    r.hosts.join(" "), r.status,
                  ]),
                )
              }
