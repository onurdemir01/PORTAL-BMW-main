...
function IpCell({ e }: { e: RouteStatsEnv }) {
  ...
  return (
    <div className="space-y-0.5">
      {shown.map((x) => (
        <div key={x.ip} className="flex items-center gap-1.5 whitespace-nowrap" title={`örnek: ${x.samples.join(', ')}`}>
          <span className="font-mono text-[11px]" style={{ color: 'var(--text-primary)' }}>{x.ip}</span>
            ...
            {rows.map(({ env, cov: c, route: r }) => {
              ...
              return (
                <tr key={env} className="border-t align-top" style={{ borderColor: 'var(--border-subtle)' }}>
                  ...
                  <td className="px-3 py-2.5">
                    {r ? (
                      <div className="space-y-0.5">
                        ...
                        <div className="text-[11px]" style={{ color: 'var(--text-muted)' }}>
                          SPA <b style={{ color: 'var(--text-secondary)' }}>{fmtNumber(r.spa)}</b> ({pctText(pct(r.spa, r.routes))}) · SPA değil {fmtNumber(r.nonSpa)}
                  ...
                  <td className="px-3 py-2.5">{r ? <IpCell e={r} /> : <span style={{ color: 'var(--text-muted)' }}>—</span>}</td>
                </tr>
              );
            })}
            {rows.length === 0 && (
