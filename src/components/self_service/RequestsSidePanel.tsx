...
export default function RequestsSidePanel() {
      ...
      if (r.ok) {
        setDetailCache((prev) => ({
          ...
          [id]: {
            ...
            awxServerId: r.awxServerId,
          },
        }));
      }
    } catch {
      /* detay alinamazsa sessizce yoksay - panel yine de kapatilabilir */
    } finally {
  ...
  const loadJobOutput = async (ticketId: number, serverId: number, jobId: number) => {
    if (jobOutputOpenId === ticketId) {
      setJobOutputOpenId(null);
      return;
    }
    ...
    try {
      ...
      if (r.ok) {
        setJobOutputCache((prev) => ({
          ...prev,
          [ticketId]: { status: r.status, output: r.output || '' },
        }));
      }
    } catch {
      /* yoksay - buton tekrar denemeye izin verir */
    } finally {
      setJobOutputLoadingId(null);
    }
  };
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="font-semibold text-sm truncate">
                            {t.templateName || `Talep #${t.id}`}
                          </div>
                          <ChevronDownIcon
                            className={`w-3.5 h-3.5 mt-0.5 flex-shrink-0 text-gray-400 transition-transform ${isExpanded ? 'rotate-180' : ''}`}
                          />
                        </div>
                        ...
                        <div className="flex items-center justify-between gap-2 mt-2">
                          <span
                            className={`text-[11px] font-semibold px-2 py-0.5 rounded-lg border ${meta.className}`}
                          >
                            {meta.label}
                          </span>
                        </div>
                            ...
                            <div className="space-y-1.5 text-[11px]">
                              {detail.externalTicketId && (
                                <div>
                                  ...
                                  <span className="font-mono">{detail.externalTicketId}</span>
                                </div>
                              )}
                              {detail.flowKey && (
                                <div>
                              ...
                              <div className="font-semibold text-gray-700 pt-1">
                                Girilen Bilgiler:
                              </div>
                              {detail.extraVars && Object.keys(detail.extraVars).length > 0 ? (
                                ...
                                <div className="text-gray-500">
                                  Bu talepte kullanıcı girdisi (extraVars) yok.
                                </div>
                              )}
                              ...
                              {t.status === 'LAUNCHED' && t.jobId && detail.awxServerId && (
                                <div className="pt-2">
                                  <button
                                    type="button"
                                    onClick={() =>
                                      loadJobOutput(t.id, detail.awxServerId!, t.jobId!)
                                    }
            ...
            (() => {
              ...
              return (
                <div
                  ...
                  onClick={(e) => {
                    if (e.target === e.currentTarget) setJobOutputOpenId(null);
                  }}
                >
                    ...
                    <div className="flex items-center justify-between gap-3 px-5 py-3 border-b border-[var(--border)] flex-shrink-0">
                      <div className="min-w-0">
                        <div
                          ...
                          title={t?.templateName || 'Job Çıktısı'}
                        >
                          {t?.templateName || 'Job Çıktısı'}
                        </div>
                        <div className="text-xs" style={{ color: 'var(--text-muted)' }}>
                          Job #{t?.jobId}
                          {t?.externalTicketId ? ` · Smart #${t.externalTicketId}` : ''}
