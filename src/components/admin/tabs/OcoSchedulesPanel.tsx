...
const STATUS_META: Record<string, { label: string; className: string }> = {
  AWX_SCHEDULED: {
    label: "AWX'e zamanlandı",
    className: 'bg-blue-50 text-blue-700 border-blue-200',
  },
  SCHEDULED: {
    label: 'Portal zamanladı',
...
export default function OcoSchedulesPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!r.ok) {
        setErr(r.message || 'Liste alınamadı.');
        return;
      }
      setRows(r.items || []);
      setTotal(r.total || 0);
  ...
  async function adminCancel(r: AdminOcoSchedule) {
    const willStopJob =
      r.status === 'LAUNCHED' ||
      (r.status === 'AWX_SCHEDULED' && new Date(r.runAt).getTime() <= Date.now());
    const msg = [
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(res.message || 'İptal edilemedi.');
        return;
      }
  ...
  const pending = rows.filter(
    (r) => r.status === 'AWX_SCHEDULED' || r.status === 'SCHEDULED',
  ).length;
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] px-4 py-3">
        <p className="text-sm text-[var(--text-secondary)] leading-relaxed">
          OCO kesinti saatine zamanlanmış işler (Self Service, ScaleX…). <b>AWX'e zamanlandı</b>{' '}
          olanları AWX'in kendi schedule'ı tetikler — Portal o anda kapalı olsa bile çalışırlar.
          <b> Portal zamanladı</b> olanlar yalnızca serviste Smart onayı da gerektiğinde oluşur;
          onları Portal'ın poller'ı tetikler, yani Portal o saatte ayakta olmalıdır.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          onClick={load}
          className="flex items-center gap-1.5 px-3 py-2 text-sm rounded-xl border border-[var(--border)] hover:bg-[var(--bg-elevated)]"
        >
          ...
          <tbody>
            {rows.map((r) => {
              ...
              return (
                <tr
                  key={r.id}
                  className="border-t border-[var(--border)] hover:bg-[var(--bg-elevated)] align-top"
                >
                  ...
                  <td className="px-3 py-2 whitespace-nowrap">
                    <span
                      className={
                        r.module === 'ScaleX'
                          ? 'pf-label pf-label--blue'
                          : 'pf-label pf-label--grey'
                      }
                    >
                      {r.module || '—'}
                    </span>
                  ...
                  <td className="px-3 py-2">
                    ...
                    {r.errorMessage && (
                      <div className="text-xs text-red-600 mt-0.5">{r.errorMessage}</div>
                    )}
                  </td>
                  <td className="px-3 py-2">
                    <span className="font-mono tabular-nums">{r.ocoNumber}</span>
                    {r.ocoSubject && (
                      <div className="text-xs text-[var(--text-muted)] mt-0.5">{r.ocoSubject}</div>
                    )}
                  </td>
                  <td className="px-3 py-2 tabular-nums whitespace-nowrap">
                    {fmt(r.runAt)}
                    {waiting && (
                      <div className="text-xs text-[var(--text-muted)]">{relative(r.runAt)}</div>
                    )}
                  </td>
                  <td className="px-3 py-2 tabular-nums whitespace-nowrap text-[var(--text-muted)]">
                    {fmt(r.windowEnd)}
                  </td>
                  <td className="px-3 py-2">
                    <span
                      className={`inline-block text-[11px] font-semibold px-2 py-0.5 rounded-full border ${meta.className}`}
                    >
                      {meta.label}
                    </span>
                  </td>
                  <td className="px-3 py-2 text-xs text-[var(--text-muted)] whitespace-nowrap">
                    ...
                    {r.cancelledBy && (
                      <div className="mt-0.5 text-[11px]">
                        iptal: {r.cancelledBy}
                        {r.cancelNote && (
                          <div className="text-[10px] opacity-80">{r.cancelNote}</div>
                        )}
                      </div>
                    )}
                  </td>
                  <td className="px-3 py-2 whitespace-nowrap">
                    {['SCHEDULED', 'AWX_SCHEDULED', 'PENDING_APPROVAL', 'LAUNCHED'].includes(
                      r.status,
                    ) && (
                      <button
                        type="button"
            ...
            {rows.length === 0 && !loading && (
              <TableEmptyRow
                colSpan={10}
                title="Zamanlanmış iş yok."
                description="OCO penceresine zamanlanan işler burada listelenir."
              />
            )}
          </tbody>
        </table>
