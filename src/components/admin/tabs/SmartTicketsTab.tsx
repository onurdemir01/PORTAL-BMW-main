...
const STATUS_META: Record<string, { label: string; className: string }> = {
  ...
  LAUNCHING: {
    label: 'Onaylandı — başlatılıyor',
    className: 'bg-blue-50 text-blue-700 border-blue-200',
  },
  LAUNCHED: {
    label: 'Onaylandı — Çalıştırıldı',
...
export default function SmartTicketsTab() {
  ...
  const load = useCallback(
    async (nextOffset = offset) => {
      setLoading(true);
      try {
        const r = await ansibleApi.smartTicketsAll({
          limit: PAGE_SIZE,
          offset: nextOffset,
          status,
          username,
          ...
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div>
              ...
              <p className="text-xs text-gray-500 max-w-2xl">
                Portal üzerinden açılan tüm Smart onay taleplerinin kalıcı kaydı (Self Service,
                ScaleX…): kim açtı, hangi otomasyon için, hangi Smart kaydı oluştu, ne zaman ve
                sonucu ne oldu. Bir satıra tıklayınca o talebin gönderilen parametreleri açılır.
              </p>
            </div>
          ...
          <div className="overflow-x-auto rounded-xl border border-gray-100">
            <table className="w-full text-sm pf-table-sticky">
              ...
              <tbody className="divide-y divide-gray-50">
                ...
                {!loading && rows.length === 0 && <TableEmptyRow colSpan={10} />}
                {rows.map((t) => {
                      ...
                      >
                        ...
                        <td className="px-3 py-2 text-xs text-gray-800">
                          <div className="flex items-center gap-1.5">
                            ...
                            />
                            {t.templateName || (
                              <span className="text-gray-300 italic">isimsiz</span>
                            )}
                          </div>
                        </td>
                        ...
                        <td className="px-3 py-2">
                          <span
                            className={`text-[11px] font-semibold px-2 py-0.5 rounded-lg border whitespace-nowrap ${meta.className}`}
                          >
                            {meta.label}
                          </span>
                          {t.smartStateName && (
                        ...
                        <td className="px-3 py-2 text-xs text-gray-600 whitespace-nowrap tabular-nums">
                          {duration(t.createdAt, t.resolvedAt)}
                        </td>
                        <td className="px-3 py-2 text-xs font-mono text-gray-700">
                          {t.jobId ? `#${t.jobId}` : '—'}
                        </td>
                        ...
                        <tr className="bg-gray-50/50">
                          <td colSpan={10} className="px-4 py-3">
                            <div className="space-y-2 text-xs">
                              ...
                              {t.errorMessage && (
                                <div className="text-red-600 bg-red-50 border border-red-100 rounded-lg px-2.5 py-1.5">
                                  {t.errorMessage}
                                </div>
                              )}
                              {(t.cancelNote || t.cancelledBy) && (
                                <div className="bg-white border border-gray-200 rounded-lg px-2.5 py-1.5">
                                  ...
                                  {t.cancelledBy && (
                                    <span className="text-gray-400"> · {t.cancelledBy}</span>
                                  )}
                                  <div className="text-gray-700 mt-0.5">
                                    {t.cancelNote || (
                                      <span className="text-gray-400 italic">not yazılmamış</span>
                                    )}
                                  </div>
                                </div>
                              )}
                              <div>
                                ...
                                {t.extraVars && Object.keys(t.extraVars).length > 0 ? (
                                  ...
                                  <span className="text-gray-400">
                                    Bu talepte kullanıcı girdisi yok.
                                  </span>
                                )}
                              </div>
                            </div>
              ...
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-5 space-y-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    ...
                    <span className="font-semibold">
                      {cancelTarget.templateName || `#${cancelTarget.id}`}
                    </span>{' '}
                    talebi.
                    {cancelTarget.externalTicketId && (
                      <>
                        {' '}
                        Smart kaydı{' '}
                        <span className="font-mono">#{cancelTarget.externalTicketId}</span>.
                      </>
                    )}
                  </p>
                </div>
                ...
                <div className="flex gap-2 items-start bg-amber-50 border border-amber-100 rounded-xl px-3 py-2">
                  ...
                  <p className="text-xs text-amber-800">
                    Bu işlem otomasyonun tetiklenmesini <b>kesin olarak</b> engeller — Smart
                    sonradan onaylansa bile iş başlamaz. Ancak{' '}
                    <b>Smart tarafındaki kayıt açık kalır</b>; Smart'ta iptal eden bir servis ucu
                    bulunmadığı için o kaydı Smart ekranından ayrıca kapatmanız gerekir.
                  </p>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">
                    İptal notu <span className="font-normal text-gray-400">(opsiyonel)</span>
                  </label>
                  <textarea
                    value={cancelNote}
