...
export default function HistoryScopePanel() {
  ...
  async function pickTable(t: string) {
    ...
    try {
      ...
      /* kolonlar alinamazsa kullanici elle secemez; sessiz gecilir */
    }
  }
      ...
      <div className="flex items-start gap-2">
        ...
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-0.5 max-w-3xl">
            Açtığınız tablo için Portal her gün bir anlık görüntü alır; tablo Envanter'de "Geçmiş"
            düğmesi kazanır ve Denetim &gt; Envanter Değişim'de listelenir.
            <strong> Geçmiş açıldığı andan itibaren birikir</strong> — geriye dönük veri üretilemez,
            çünkü envanter tabloları her yenilemede sıfırdan yazılıyor.
          </p>
        </div>
      </div>
                ...
                <tr key={r.table} className="border-t border-gray-100">
                  ...
                  <td className="px-3 py-2 text-right">
                    <button
                      ...
                      title={
                        r.enabled
                          ? 'Geçmiş tutuluyor — kapatmak için tıklayın'
                          : 'Geçmişi tutmaya başla'
                      }
                    >
                      {r.enabled ? (
        ...
        {newTable && (
          <div>
            <p className="text-[11px] text-gray-500 mb-1">
              <strong>Anahtar kolonları seçin</strong> — birlikte bir satırı tekil olarak tanımlayan
              kolonlar (ör. <code>host</code> + <code>app</code>). Otomatik seçilmiyor: tabloların
              birincil anahtarı her yenilemede baştan üretilen bir
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              {newCols.length === 0 && (
                <span className="text-[11px] text-gray-400">Kolonlar okunuyor…</span>
              )}
            </div>
          </div>
        )}
