...
export default function PageVisibilityTab() {
  ...
  async function createElement() {
    if (!newEl.key.trim()) {
      toast.error('element_key gerekli.');
      return;
    }
    try {
      await elementsApi.create({
        key: newEl.key.trim(),
        type: newEl.type,
        label: newEl.label.trim() || newEl.key.trim(),
        parentKey: newEl.parentKey.trim() || null,
        enabled: true,
        defaultVisible: true,
  ...
  async function removeElement(key: string) {
    if (!window.confirm(`"${key}" öğesini ve tüm görünürlük kurallarını silmek istiyor musunuz?`))
      return;
    try {
        ...
        <tr className="hover:bg-gray-50/50 border-b border-gray-50">
          <td className="px-3 py-2.5" style={{ paddingLeft: `${12 + depth * 20}px` }}>
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] px-1.5 rounded bg-gray-100 text-gray-500">
                {el.type}
              </span>
              {el.description && (
                <span
                  className="text-[10px] text-gray-400 italic truncate max-w-[240px]"
          ...
          <td className="px-3 py-2.5 text-center">
            <button
              ...
              title={
                e.enabled
                  ? 'Aktif — kapatmak için tıkla (herkese, admin dahil, kapanır)'
                  : 'Kapalı — açmak için tıkla'
              }
            >
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500 max-w-2xl">
            Her sayfa/tab/buton için: <b>Aktif</b> = global kill-switch (kapatınca{' '}
            <i>admin dahil</i> hiçbir yerde görünmez), <b>User görür</b> = User rolü görebilir mi
            (Admin her zaman görür), ve kullanıcı-bazlı override. Değişiklikler kaydedilince{' '}
            <b>anında</b> yansır (reload gerekmez).
          </p>
        </div>
      ...
      {adding && (
        <div className="border border-blue-100 bg-blue-50/40 rounded-xl p-3 grid grid-cols-2 md:grid-cols-4 gap-2 items-end">
          <label className="text-xs text-gray-600 space-y-1">
            ...
            <input
              value={newEl.key}
              onChange={(e) => setNewEl({ ...newEl, key: e.target.value })}
              placeholder="btn:logx:cancel"
          ...
          <label className="text-xs text-gray-600 space-y-1">
            ...
            <Select
              sizeVariant="sm"
              value={newEl.type}
              onChange={(e) => setNewEl({ ...newEl, type: e.target.value })}
            >
              {['page', 'tab', 'button', 'admin_tab', 'feature', 'nav_group', 'mcp', 'awx'].map(
                (t) => (
          ...
          <label className="text-xs text-gray-600 space-y-1">
            ...
            <input
              value={newEl.label}
              onChange={(e) => setNewEl({ ...newEl, label: e.target.value })}
              className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1.5"
            />
          </label>
          <div className="flex gap-2">
            <input
              value={newEl.parentKey}
              onChange={(e) => setNewEl({ ...newEl, parentKey: e.target.value })}
              placeholder="parent_key (ops.)"
          ...
          <button
            ...
            className="px-3 py-1.5 bg-blue-600 text-white text-xs font-medium rounded-lg hover:bg-blue-700 h-fit"
          >
            Ekle
          </button>
        </div>
      ...
      <div className="border border-gray-100 rounded-xl overflow-hidden">
        <table className="w-full text-sm pf-table-sticky">
          ...
          <tbody>{roots.map((el) => renderRow(el, 0))}</tbody>
        </table>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-3xl w-full p-6 space-y-6">
        <div className="flex items-start justify-between">
          <h3 className="text-base font-semibold text-gray-800">
            Görünürlük & Modüller — Nasıl Çalışır?
          </h3>
        ...
        <div className="space-y-3 text-sm text-gray-700 leading-relaxed">
          <p>
            <b>Aktif (kill-switch):</b> Kapatılırsa bu öğe <i>admin dahil</i> hiç kimseye görünmez.
            Bir modülü tamamen devre dışı bırakmak için kullanılır (ör. bakım sırasında).
          </p>
          <p>
            <b>User görür:</b> "User" rolündeki kullanıcıların bu öğeyi görüp göremeyeceğini
            belirler. Admin rolü, öğe Aktif olduğu sürece HER ZAMAN görür — bu kural Admin'i
            etkilemez.
          </p>
          <p>
            <b>Kullanıcı override:</b> Belirli bir kullanıcı için rol kuralının üzerine yazar.
            "allow" değeri true ise o kullanıcı rol kuralı ne olursa olsun görür; false ise rol
            kuralı ne olursa olsun görmez.
          </p>
          <p>
            <b>Üst-alt (parent-child) ilişkisi:</b> Bir sayfa (page) kapatılırsa, altındaki tüm
            tab/buton/admin_tab öğeleri de otomatik olarak görünmez olur — alt öğeleri tek tek
            kapatmaya gerek yoktur.
          </p>
          <p>
            <b>Değişikliklerin yansıması:</b> "Kaydet" tıklandığı an tüm açık oturumlarda anında
            geçerli olur (sayfa yenilemesi/yeniden giriş gerekmez) — motor bir versiyon sayacı
            tutar, istemciler bunu düzenli kontrol eder.
          </p>
          <p>
            <b>Yeni element nasıl eklenir:</b> "Yeni Element" ile bir element_key + tip
            (page/tab/button/admin_tab/feature/nav_group) tanımlanır. parent_key opsiyoneldir — bir
            sayfanın altına bağlamak için o sayfanın element_key'i girilir.
          </p>
          <p>
            <b>element_key nasıl belirlenir:</b> Sayfalar için sade isim (ör. <code>Dashboard</code>
            ), alt-öğeler için <code>tip:sayfa:isim</code> deseni önerilir (ör.{' '}
            <code>btn:logx:cancel</code>, <code>admintab:users</code>) — kod tarafında bu anahtarla
            eşleştiği için oluşturulduktan sonra değiştirilmemelidir.
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
              Örnek: Element tablosu (portal_elements)
            </p>
            <div className="overflow-x-auto rounded-lg border border-gray-100">
              <table className="w-full text-xs pf-table-sticky">
                <thead>
                  <tr className="bg-gray-50 text-left text-gray-500">
                    <th className="px-2 py-1.5">element_key</th>
                    <th className="px-2 py-1.5">type</th>
                    <th className="px-2 py-1.5">parent_key</th>
                    <th className="px-2 py-1.5">enabled</th>
                    <th className="px-2 py-1.5">default_visible</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50 font-mono">
                  <tr>
                    <td className="px-2 py-1.5">LogX</td>
                    <td className="px-2 py-1.5">page</td>
                    <td className="px-2 py-1.5">—</td>
                    <td className="px-2 py-1.5">1</td>
                    <td className="px-2 py-1.5">1</td>
                  </tr>
                  <tr>
                    <td className="px-2 py-1.5">admintab:users</td>
                    <td className="px-2 py-1.5">admin_tab</td>
                    <td className="px-2 py-1.5">Admin</td>
                    <td className="px-2 py-1.5">1</td>
                    <td className="px-2 py-1.5">0</td>
                  </tr>
                  <tr>
                    <td className="px-2 py-1.5">btn:logx:cancel</td>
                    <td className="px-2 py-1.5">button</td>
                    <td className="px-2 py-1.5">LogX</td>
                    <td className="px-2 py-1.5">0</td>
                    <td className="px-2 py-1.5">1</td>
                  </tr>
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
              Örnek: Kullanıcı-bazlı override (portal_element_visibility, principal_type=user)
            </p>
            <div className="overflow-x-auto rounded-lg border border-gray-100">
              <table className="w-full text-xs pf-table-sticky">
                <thead>
                  <tr className="bg-gray-50 text-left text-gray-500">
                    <th className="px-2 py-1.5">element_key</th>
                    <th className="px-2 py-1.5">principal_id (kullanıcı)</th>
                    <th className="px-2 py-1.5">allow</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50 font-mono">
                  <tr>
                    <td className="px-2 py-1.5">LogX</td>
                    <td className="px-2 py-1.5">ahmet.yilmaz</td>
                    <td className="px-2 py-1.5">0 (gizli)</td>
                  </tr>
                  <tr>
                    <td className="px-2 py-1.5">btn:logx:cancel</td>
                    <td className="px-2 py-1.5">mehmet.demir</td>
                    <td className="px-2 py-1.5">1 (görür)</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
              Örnek: Rol-bazlı kural (portal_element_visibility, principal_type=role)
            </p>
            <div className="overflow-x-auto rounded-lg border border-gray-100">
              <table className="w-full text-xs pf-table-sticky">
                <thead>
                  <tr className="bg-gray-50 text-left text-gray-500">
                    <th className="px-2 py-1.5">element_key</th>
                    <th className="px-2 py-1.5">principal_id (rol)</th>
                    <th className="px-2 py-1.5">allow</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50 font-mono">
                  <tr>
                    <td className="px-2 py-1.5">LogX</td>
                    <td className="px-2 py-1.5">User</td>
                    <td className="px-2 py-1.5">1 (görür)</td>
                  </tr>
                  <tr>
                    <td className="px-2 py-1.5">admintab:users</td>
                    <td className="px-2 py-1.5">User</td>
                    <td className="px-2 py-1.5">0 (görmez)</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div className="flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 text-xs font-medium bg-gray-900 text-white rounded-lg hover:bg-gray-800"
          >
            Kapat
          </button>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        {overrides.map((o, i) => (
          ...
          >
            ...
            <button
              onClick={() => onChange(overrides.filter((_, j) => j !== i))}
              className="text-gray-300 hover:text-red-500"
            >
