...
function ImageAssetEditor({
  ...
  const onPick = async (file: File) => {
    ...
    if (limits && file.size > limits.maxBytes) {
      setError(
        `Dosya çok büyük (${Math.round(file.size / 1024)} KB). Üst sınır ${limits.maxBytes / 1024} KB.`,
      );
      return;
    }
  ...
  return (
    <div className="space-y-4">
      ...
      {error && (
        <div className="pf-alert pf-alert--danger p-3 text-sm" role="alert">
          {error}
        </div>
      )}
      {okMsg && (
        <div className="pf-alert pf-alert--success p-3 text-sm" role="status">
          {okMsg}
        </div>
      )}
      ...
      <div className="flex items-center gap-5 flex-wrap">
        ...
        >
          {asset ? (
            <img
              src={asset.dataUrl}
              alt="Mevcut görsel"
              style={{ maxWidth: previewBox, maxHeight: previewBox }}
            />
          ) : (
        ...
        <div className="text-sm space-y-1">
          {asset ? (
            <>
              <div>
                <strong>Format:</strong> {asset.mime}
              </div>
              <div>
                <strong>Boyut:</strong> {Math.round(asset.sizeBytes / 1024)} KB
              </div>
              {asset.updatedBy && (
                <div>
                  <strong>Yükleyen:</strong> {asset.updatedBy}
                </div>
              )}
              {asset.updatedAt && (
            ...
            <div style={{ color: 'var(--text-muted)' }}>
              Henüz yüklenmedi — varsayılan kullanılıyor.
            </div>
          )}
        </div>
      </div>
      ...
      <div className="text-xs space-y-1" style={{ color: 'var(--text-muted)' }}>
        ...
        <div>
          <strong>En büyük dosya:</strong> {limits ? limits.maxBytes / 1024 : 512} KB
        </div>
        <div>
          <strong>Önerilen:</strong> {recommendation}
        </div>
        <div>
          SVG güvenlik nedeniyle kabul edilmez — çalıştırılabilir kod içerebildiği ve dosya portalla
          aynı adresten sunulduğu için.
        </div>
        <div>
          WEBP kabul edilmez — tarayıcıların işleme hattı bu formatı güvenilir şekilde çözmez; dosya
          yüklenir ama görsel değişmez.
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-xs" style={{ color: 'var(--text-muted)' }}>
        Yeni görsel bazı tarayıcılarda hemen görünmeyebilir; sekmeyi kapatıp açmak veya Ctrl+Shift+R
        yeterlidir.
      </div>
    </div>
  );
