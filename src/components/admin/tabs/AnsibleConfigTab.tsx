...
export default function AnsibleConfigTab() {
  ...
  async function reload() {
    setLoading(true);
    try {
      const [h, t] = await Promise.allSettled([ansibleApi.health(), ansibleApi.awxTemplatesAll()]);
      if (h.status === 'fulfilled') setHealth(h.value as HealthStatus);
      else setHealth({ ok: false, message: 'Bağlanamadı' });
      if (t.status === 'fulfilled') {
        const r = t.value as ReturnType<typeof ansibleApi.awxTemplatesAll> extends Promise<infer U>
          ? U
          : never;
  ...
  // IKI AYRI HOOK, BILEREK:
  //   * Ilk yukleme `useAsyncEffect` ile — `reload()` ilk isi olarak setState
  //     yapiyor ve effect govdesinde SENKRON cagrilmasi uyari uretiyordu.
  //   * Interval AYRI bir `useEffect`te kaliyor, cunku TEMIZLIK dondurmesi gerek
  //     (`useAsyncEffect` cleanup dondurmez). `setInterval` geri cagrisi zaten
  //     effect govdesinde senkron calismaz, yani orada sorun yok.
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        ...
        <div
          className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${health?.ok ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'}`}
        >
          {health?.ok ? (
          ...
          <div>
            ...
            {health?.message && !health?.ok && (
              <p className="text-xs text-red-500 mt-0.5">{health.message}</p>
            )}
          </div>
        </div>
            ...
            {health.servers.map((s) => (
              ...
              >
                ...
                {s.responseTimeMs != null && (
                  <span className="text-gray-400">{s.responseTimeMs}ms</span>
                )}
        ...
        <h3 className="text-sm font-semibold text-gray-700 mb-1">
          ...
          {totalTemplates > 0 && (
            <span className="ml-2 text-xs font-normal text-gray-400">
              ({totalTemplates} toplam)
            </span>
          )}
        </h3>
        <p className="text-xs text-gray-400 mb-3">
          Tüm AWX sunucularındaki read-only iş şablonları.
        </p>
        {loading ? (
                        ...
                        {srv.templates.map((t) => (
                          <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-4 py-2 font-medium text-gray-800 text-xs">
                              {t.name}
                              {t.hasInventory === false && (
                                <span
                                  className="ml-1.5 text-[10px] text-amber-600"
                                  title="Envanter atanmamış — launch edilemeyebilir"
                                >
                                  ⚠
                                </span>
                              )}
                            </td>
                            <td className="px-4 py-2 text-xs font-mono text-[#1A56DB]">
                              {t.playbook}
                            </td>
      ...
      <section className="px-4 py-3 bg-amber-50 border border-amber-100 rounded-xl text-xs text-amber-700">
        ...
        <ul className="space-y-1 font-mono">
          {[
            ...
            {
              key: 'AWX_USER + AWX_PASSWORD',
              desc: "AWX_TOKEN tanımlı değilse, token almak için kullanılan servis hesabı — şifre DB'de şifreli saklanır.",
            },
            {
              key: 'AWX_READ_ONLY_TEMPLATE_IDS',
              desc: "Virgülle ayrılmış template ID listesi — boşsa TÜM template'ler izinli sayılır.",
            },
            {
              key: 'AWX_LOG_FETCH_TEMPLATE_ID',
              desc: "AI Analist'in uzak log dosyası çekmek için kullandığı template ID'si.",
            },
            {
              key: 'TEAMS_LONGJOB_WEBHOOK_URL',
              desc: "Uzun süren job bildirimi ve otomatik iptal kartı bu Teams webhook'una gider (eşik: TEAMS_LONGJOB_THRESHOLD_MINUTES, varsayılan 30; iptal eşiği Admin ekranından).",
            },
          ].map((v) => (
            <li key={v.key} className="flex items-center gap-1.5">
              <span>{v.key}</span>
              <span className="text-amber-400 cursor-help font-sans" title={v.desc}>
                ⓘ
              </span>
            </li>
          ))}
        </ul>
        <p className="mt-1.5 text-amber-500 font-sans">
          Tüm değerler Admin &gt; Sistem sekmesinden de kalıcı olarak (DB'de) ayarlanabilir.
        </p>
      </section>
    </div>
  );
