...
const ENV_VARS: EnvVarMeta[] = [
  ...
  { key: "SMART_SERVICEREPOSITORY_URL", label: "ServiceRepository Adresi (DOĞRULANMADI)", group: "Smart", description: "⚠ Talep durumu, Smart'ın KENDİ API host'undan DEĞİL, ayrı bir \"ServiceRepository\" sisteminden sorgulanıyor (kardeş ekibin gerçek kaynağından çıkarıldı — dashboard/servicerepository.py, AppSettings tablosunda \"Servicerepository Address\" adıyla tutuluyor, koddaki değer sadece anahtar adı, gerçek URL orada da yok). Bu portalı işleten ekipten (LB/GSLB ekibi) alın.", required: false, example: "https://<host>:<port>", usedIn: "server/smart/client.cjs", restartRequired: false },
  { key: "SMART_CHECK_TICKET_PATH", label: "Durum Sorgulama Path'i (DOĞRULANMADI)", group: "Smart", description: "⚠ SOS02-KL-001-EN dokümanında YOK ama kardeş ekibin gerçek kodundan protokolü çıkarıldı: GET isteği, ?wfInstanceId=..&languageCode=TR. AppSettings'te \"Servicerepository Ticket Check Path\" adıyla tutuluyor, gerçek değer koddan bulunamadı. Boş bırakılırsa checkTicketStatus() açıkça hata verir, talepler PENDING'de bekler.", required: false, example: "/AgileManagement/.../LoadRoadmap", usedIn: "server/smart/client.cjs", restartRequired: false },
