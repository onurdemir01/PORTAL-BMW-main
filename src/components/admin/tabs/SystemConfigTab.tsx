...
const ENV_VARS: EnvVarMeta[] = [
  ...
  { key: "SMART_SERVICEREPOSITORY_URL", label: "ServiceRepository Adresi", group: "Smart", description: "Talep durumu Smart'ın KENDİ API host'undan DEĞİL, ayrı bir \"ServiceRepository\" sisteminden sorgulanıyor. Kullanıcı tarafından doğrulandı: https://servicerepository/ — FQDN olmayan iç DNS adı, kurumsal proxy'den (TEKPRXV2) muaf tutulur (kod bunu otomatik uygular, SMART_PROXY_URL bu isteğe uygulanmaz). Boş bırakılırsa checkTicketStatus() açıkça hata verir, talepler PENDING'de bekler.", required: false, example: "https://servicerepository", usedIn: "server/smart/client.cjs", restartRequired: false },
  { key: "SMART_CHECK_TICKET_PATH", label: "Durum Sorgulama Path'i", group: "Smart", description: "DOĞRULANDI — kardeş ekibin gerçek test scriptinden (dashboard/deneme.py) birebir alındı: GET isteği, ?wfInstanceId=..&languageCode=TR. Varsayılan değer zaten doğru path'i içeriyor, genelde değiştirmenize gerek yok.", required: false, example: "/Workflow/WorkflowServiceRepository.svc/Workflow/LoadRoadmap/", usedIn: "server/smart/client.cjs", restartRequired: false },
