...
function LinkForm({
  ...
  // NE ISE YARAR / NASIL KULLANILIR. Kullanici istegi: portal "super app" gibi
  // yonlendirirken gidilen yerin ne oldugu ve nasil kullanildigi da yazsin.
  ...
  const [visibleTo, setVisibleTo] = useState<('Admin' | 'User')[]>(
    initial?.visibleTo ?? ['Admin', 'User'],
  );
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <textarea
          ...
          rows={2}
          className="input-modern"
          placeholder="Bu bağlantı hangi ihtiyacı çözer?"
        />
      </div>
      <div>
        <label
        ...
        <textarea
          ...
          rows={3}
          className="input-modern"
          placeholder="Giriş nasıl yapılır, hangi adımlar izlenir, kime sorulur…"
        />
      </div>
      ...
      <div className="flex gap-2 justify-end pt-2">
        ...
        <button
          type="submit"
          disabled={loading || !label.trim() || !url.trim() || !category.trim()}
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm font-medium mt-1" style={{ color: 'var(--text-muted)' }}>
            Sık kullanılan sistem bağlantıları
          </p>
        </div>
        {isAdmin && (
          ...
          {filtered.map((link) => (
            ...
            >
              ...
              <div>
                ...
                {/* AYRINTI: kartta yer kaplamasin, isteyen acsin. "Ne ise yarar" ve
                    "nasil kullanilir" kartin kendisinde olsaydi liste okunmaz olurdu. */}
                {(link.purpose || link.howToUse) && (
                  <details className="mt-1.5">
                    <summary className="text-xs cursor-pointer" style={{ color: 'var(--accent)' }}>
                      Ne işe yarar / nasıl kullanılır
                    </summary>
                    {link.purpose && (
                      <p
                        className="text-xs mt-1.5 whitespace-pre-line"
                        style={{ color: 'var(--text-secondary)' }}
                      >
                        {link.purpose}
                      </p>
              ...
              >
                {/* SUNUCU KAPISI TEK BASINA YETMEZ. O kapi yalnizca YENI yazimlari
                    korur; tabloda ZATEN duran satirlar dogrulanmadan gecmistir
                    (`javascript:` iceren eski bir kayit hala burada render edilirdi).
                    Ayni kural render aninda da uygulanir. */}
                {(() => {
                  const su = safeLinkUrl(link.url);
                  if (!su.safe) {
                    return (
                      <span
