// src/components/telnet/steps/OcpTargetStep.tsx — Openshift hedefi: ortam → tenant →
// cluster(lar) + namespace. OpsX'in OcpTargetStep'inden TEK FARKI: "Uygulama Adı"
// alanı YOK (kullanıcı isteği — Telnet testi için gerekli değil).
import React, { useEffect, useState } from "react";
import { telnetApi } from "@/api/telnetApi";

const OcpTargetStep: React.FC<{
  busy?: boolean;
  onSubmit: (v: { env: string; tenant: string; clusters: string[]; namespace: string }) => void;
}> = ({ busy, onSubmit }) => {
  ...
  const ready = env && tenant && clusters.size > 0 && namespace.trim();
  ...
  return (
    <div className="space-y-4">
      ...
      <button
        onClick={() => onSubmit({ env, tenant, clusters: [...clusters], namespace: namespace.trim() })}
        disabled={!ready || busy}
