            ...
            {view.length === 0 ? <TableEmptyRow colSpan={9} title={hosts.length ? 'Süzgeçle eşleşen sunucu yok.' : 'Envanterde nginx sunucusu yok.'} /> : view.map((h) => {
              ...
              return (
                <tr key={h.host} className="border-t hover:bg-[var(--bg-elevated)]" style={{ borderColor: 'var(--border-subtle)' }}>
                  ...
                  <td className="px-3 py-2.5"><button className="underline font-medium" style={{ color: 'var(--accent)' }} onClick={() => onOpen(h.host)}>{h.host.toLowerCase()}</button>{h.inventoryMissing && <span className="ml-1 text-[10px]" style={{ color: 'var(--text-muted)' }}>(envanterde yok)</span>}{h.dumpTooLarge ? <span className="ml-1 text-[10px] px-1 py-0.5 rounded" style={{ background: 'var(--status-warning-bg)', color: 'var(--status-warning)' }} title={`Dokum ${h.dumpTooLarge} MB — özet (ağaç, sertifika, nginx -t) okunuyor ama dosya içerikleri bu ekranda açılamıyor.`}>dokum {h.dumpTooLarge} MB</span> : null}</td>
