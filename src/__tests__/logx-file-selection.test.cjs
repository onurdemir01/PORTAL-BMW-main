...
const DOWNLOAD = fs.readFileSync(
  path.join(__dirname, '..', 'components', 'logx_v2', 'shared', 'DownloadStep.tsx'), 'utf8'
);

test('D2: liste sanallastirilmis (FileX ile AYNI kutuphane)', () => {
  assert.match(SRC, /from "@tanstack\/react-virtual"/, 'sanallastirma yok — 15.000 satir DOM’a basilir');
  assert.match(SRC, /useVirtualizer\(\{/);
  assert.match(SRC, /getScrollElement: \(\) => scrollRef\.current/);
});
...
test('D4: arama dogrudan degil, ERTELENMIS degerle suzuyor', () => {
  ...
  assert.match(SRC, /const searchPending = rawQuery !== query;/, 'kullanici suzmenin surdugunu gormeli');
});
...
test('D3: gonderilecek GOVDE olculuyor ve sinir asilirsa buton kapali', () => {
  ...
  assert.match(SRC, /disabled=\{selected\.size === 0 \|\| busy \|\| overLimit\}/,
...
test('EK: log tipi rozeti, son-24-saat suzgeci ve grup toplami var', () => {
  ...
  assert.match(SRC, /row\.bytes > 0 && ` · \$\{fmtSize\(row\.bytes\)\}`/, 'dizin/host grup toplami gosterilmiyor');
});
...
test('EK: indirme ekraninda sizeBytes GOSTERILIYOR', () => {
  ...
  assert.match(DOWNLOAD, /items\.reduce\(\(n, it\) => n \+ \(it\.sizeBytes \|\| 0\), 0\)/,
...
test('mtime tipi duzeltilmis (number | string)', () => {
  ...
  assert.match(api, /mtime\?: number \| string;/,
