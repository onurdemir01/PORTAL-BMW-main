...
async function json<T>(r: Response): Promise<T> {
  if (!r.ok) {
    ...
    throw Object.assign(new Error(String(message).slice(0, 300) || `HTTP ${r.status}`), {
      status: r.status,
    });
  }
  return safeJson(r);
...
export interface LogXv2Request {
  ...
  discoveryResult:
    LegacyDiscoveryResult | OcpNamespaceDiscoveryResult | Record<string, unknown> | null;
...
export const logxV2Api = {
  createRequest: (platform: Platform) =>
    postJson<{ ok: boolean; requestId: string }>('/requests', { platform }),

  getRequest: (requestId: string) =>
    fetch(`${BASE}/requests/${requestId}`).then((r) =>
      json<{
        ok: boolean;
        request: LogXv2Request;
        jobs: LogXv2Job[];
  ...
  legacyHosts: (app: string) =>
    fetch(`${BASE}/legacy/hosts?app=${encodeURIComponent(app)}`).then((r) =>
      json<{ ok: boolean; hosts: LegacyHost[] }>(r),
    ),

  searchLegacyApps: (search: string) =>
    fetch(`${BASE}/legacy/apps?search=${encodeURIComponent(search)}`).then((r) =>
      json<{ ok: boolean; apps: string[]; fallbackMode: boolean }>(r),
    ),
  ...
  // ── OpenShift ──────────────────────────────────────────────────────────────
  getClusterTree: () =>
    fetch(`${BASE}/ocp/cluster-index`).then((r) =>
      json<{ ok: boolean; tree: Record<string, Record<string, string[]>> }>(r),
    ),

  selectClusters: (requestId: string, env: string, tenant: string, clusters: string[]) =>
    postJson<{ ok: boolean; terminalHost: string }>(`/ocp/${requestId}/select`, {
      env,
      tenant,
      clusters,
    }),
  ...
  cachedNamespaces: (env: string, tenant: string, cluster: string) =>
    fetch(
      `${BASE}/ocp/cache/namespaces?env=${encodeURIComponent(env)}&tenant=${encodeURIComponent(tenant)}&cluster=${encodeURIComponent(cluster)}`,
    ).then((r) => json<CachedList<string>>(r)),

  cachedApps: (env: string, tenant: string, cluster: string, namespace: string) =>
    fetch(
      `${BASE}/ocp/cache/apps?env=${encodeURIComponent(env)}&tenant=${encodeURIComponent(tenant)}&cluster=${encodeURIComponent(cluster)}&namespace=${encodeURIComponent(namespace)}`,
  ...
  inventoryNamespaces: (env: string, tenant: string, clusters: string[]) =>
    fetch(
      `${BASE}/ocp/inventory/namespaces?env=${encodeURIComponent(env)}&tenant=${encodeURIComponent(tenant)}&clusters=${encodeURIComponent(clusters.join(','))}`,
    ).then((r) => json<CachedList<string>>(r)),
  ...
  playbookReadiness: () =>
    fetch(`${BASE}/playbook-readiness`).then((r) =>
      json<{ ok: boolean; rows: PlaybookReadiness[] }>(r),
    ),
  ...
  jobStatus: (jobId: number) =>
    ...
    fetch(`${BASE}/jobs/${jobId}/status`).then((r) =>
      json<{
        ok: boolean;
        status: string;
        jobType: string;
  ...
  resetRequest: (
    requestId: string,
    to: 'legacy_app' | 'ocp_cluster_select' | 'ocp_namespace_step',
  ...
  admin: {
    ...
    createClusterIndex: (data: Partial<OcpClusterIndexRow>) =>
      postJson<{ ok: boolean; row: OcpClusterIndexRow }>('/admin/ocp-cluster-index', data),
    updateClusterIndex: (id: number, data: Partial<OcpClusterIndexRow>) =>
      putJson<{ ok: boolean; row: OcpClusterIndexRow }>(`/admin/ocp-cluster-index/${id}`, data),
    ...
    clusterPodStatus: (id: number, body: { namespace?: string; labelSelector?: string } = {}) =>
      postJson<{ ok: boolean; output?: string; jobId?: number; jumpHost?: string }>(
        `/admin/ocp-cluster-index/${id}/pod-status`,
        body,
      ),
    ...
    listVaultKeys: () =>
      fetch(`${BASE}/admin/ocp-vault-keys`).then((r) =>
        json<{ ok: boolean; rows: OcpVaultKeyRow[] }>(r),
      ),
    createVaultKey: (data: Partial<OcpVaultKeyRow>) =>
      postJson<{ ok: boolean; row: OcpVaultKeyRow }>('/admin/ocp-vault-keys', data),
    updateVaultKey: (id: number, data: Partial<OcpVaultKeyRow>) =>
      putJson<{ ok: boolean; row: OcpVaultKeyRow }>(`/admin/ocp-vault-keys/${id}`, data),
    ...
    getPlaybookReadiness: () =>
      fetch(`${BASE}/admin/playbook-readiness`).then((r) =>
        json<{ ok: boolean; rows: PlaybookReadinessRow[] }>(r),
      ),

    listTerminalHostMap: () =>
      fetch(`${BASE}/admin/ocp-terminal-host-map`).then((r) =>
        json<{ ok: boolean; rows: OcpTerminalHostRow[] }>(r),
      ),
    createTerminalHost: (data: Partial<OcpTerminalHostRow>) =>
      postJson<{ ok: boolean; row: OcpTerminalHostRow }>('/admin/ocp-terminal-host-map', data),
    updateTerminalHost: (id: number, data: Partial<OcpTerminalHostRow>) =>
      putJson<{ ok: boolean; row: OcpTerminalHostRow }>(`/admin/ocp-terminal-host-map/${id}`, data),
    ...
    getOcpRuntimeConfig: () =>
      fetch(`${BASE}/admin/ocp-runtime-config`).then((r) =>
        json<{ ok: boolean; config: OcpRuntimeConfig; defaults: OcpRuntimeConfig }>(r),
      ),
    saveOcpRuntimeConfig: (config: OcpRuntimeConfig) =>
      putJson<{ ok: boolean; config: OcpRuntimeConfig }>('/admin/ocp-runtime-config', config),

    listEnvSuffixMap: () =>
      fetch(`${BASE}/admin/env-suffix-map`).then((r) =>
        json<{ ok: boolean; rows: EnvSuffixRow[] }>(r),
      ),
    createEnvSuffix: (data: Partial<EnvSuffixRow>) =>
      postJson<{ ok: boolean; row: EnvSuffixRow }>('/admin/env-suffix-map', data),
    ...
    listRequests: (params: { state?: string; platform?: string } = {}) => {
      ...
      return fetch(`${BASE}/admin/requests${qs ? `?${qs}` : ''}`).then((r) =>
        json<{ ok: boolean; requests: LogXv2Request[] }>(r),
      );
    },
  },
};
...
export interface OcpTerminalHostRow {
  id: number;
  tenant: string;
  env: string;
...
export interface CachedList<T> {
  ...
  /** Tarama KAYDI OKUNAMADI (tablo yok / DB hatası). "Hiç taranmadı" ile aynı ŞEY DEĞİL:
   *  taradıktan sonra da kayıt okunamayacağı için otomatik tarama sonsuza kadar
   *  tekrarlanırdı. Sihirbaz bu durumda otomatik tarama YAPMAZ, elle tarama açık kalır. */
  scanUnknown?: boolean;
}

/** Log çekilecek bir (namespace, uygulama) çifti. */
export interface OcpFetchTarget {
  namespace: string;
  appName: string;
}
