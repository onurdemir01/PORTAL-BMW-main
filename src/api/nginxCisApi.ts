...
export interface NcCisHostRow {
  host: string; nginxVersion: string | null; tState: string | null; scanDate: string | null; scannedAt?: string | null;
  score: number | null; passed: number; failed: number; excepted: number; skipped: number;
  /** 2026-09-24: sunucu KOMPLE istisnada - her maddesi istisna, skoru null, ortalamaya girmez */
  exceptedHost?: boolean; exceptionNote?: string | null;
}
...
/** Bir sunucuyu KOMPLE istisnaya almak icin item_id yerine kullanilan sentinel (2026-09-24). */
export const CIS_HOST_ALL = '*';
...
export interface NcCisOverview {
  ...
  summary: { hosts: number; scanDate: string | null; scannedAt?: string | null; avgScore: number | null; under80: number; perfect: number; failCells: number; exceptedCells: number; exceptedHosts?: number; items: number; tFail: number } | null;
